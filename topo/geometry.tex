...
ただし，\(H^n\)は\(n\)次元上半空間\[
  ...
  \right\}
\]を表す．このとき，
\(T_{x}\bd{X}\)は\(T_{x}X\)において余次元\(1\)の部分ベクトル空間
である．\(T_{x}X\)のベクトル\(v\in T_{x}X\)で，
接写像\(d_{x}\varphi\colon T_{x}U\to T_{\varphi(x)}H^n\)による
像\(d_{x}\varphi(v)\)が\(\Int{H^n}\)に属するとき，
\(v\)は\(x\in\bd{X}\)における内向きのベクトルであるという．
\(-d_{x}\varphi(v)\)が\(\Int{H^n}\)に属するとき，
\(v\)は\(x\in\bd{X}\)における外向きのベクトルであるという．

標準双線形形式\(
  \langle\blk,\blk\rangle\colon T_{x}X\times T_{x}^{\ast}X\to\rr
\)を考える．
%の\(T_{x}\bd{X}\)への
%制限\(T_{x}\bd{X}\times T_{x}^{\ast}X\to\rr\)
\(\xi\in T_{x}^{\ast}X\)が次の2つの条件をみたすとき，
\(\xi\)は\(x\in\bd{X}\)における
外向き余法ベクトルであるという．
\begin{enumerate}
  \item \(\xi\)の双対ベクトル\(v\in T_{x}X\)は\(x\in\bd{X}\)における外向きのベクトルである．
  \item 任意の\(v\in T_{x}\bd{X}\)に対し\(\langle v,\xi\rangle=0\)である．
\end{enumerate}
