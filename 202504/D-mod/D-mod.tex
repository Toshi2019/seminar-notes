...
\title{\(D\)加群ノート}
\author{Toshi2019}
\date{\today 更新\footnote{2025/04/05作成開始}}
...
% documentclass
% ------------------------
\documentclass[9pt, a4paper, dvipdfmx, leqno, draft]{jsreport}
...
% 複素多様体上の層
\newcommand{\funHol}{\mathcal{O}}
\newcommand{\vecHol}{\varTheta}
\newcommand{\formHol}{\varOmega}
\newcommand{\diffHol}{\mathcal{D}}

\newcommand{\nnum}{\mathbf{N}}
\newcommand{\znum}{\mathbf{Z}}
\newcommand{\qnum}{\mathbf{Q}}
\newcommand{\rnum}{\mathbf{R}}
\newcommand{\cnum}{\mathbf{C}}
...
%\layout
\chapter*{はじめに}
2025年2月から始めた\cite{Kas08}のセミナーのノート．
...
\begin{itemize}
    ...
    \item 非負整数の集合を\(\znum_{\geqq0}\)で表す．
\end{itemize}
\tableofcontents
\chapter*{序論}

\chapter{\(D\)加群の簡単な性質}\label{chap1}
\section{\(D\)加群}\label{sec11}
\(X\)を\(n\)次元の複素多様体とする．
\(X\)上の正則関数のなす層を\(\funHol_X\)で表す．

\(\cnum_X\)加群の射\(P\colon\funHol_X\to\funHol_X\)で，
局所的に\[
    P(x)=\sum_{\alpha\in\znum_{\geqq0}^{n}}^{}
    a_{\alpha}(x)\partial^{\alpha}
\]とかけるものを（正則）微分作用素という．

\(X\)上の正則関数の層\(\funHol_X\)と
正則ベクトル場の層\(\vecHol_X\)の生成する
\(\HOM_{\cnum_{X}}(\funHol_X,\funHol_X)\)の部分環を\(\diffHol_X\)で表し，
（正則な）\textbf{微分作用素} (differential operator) の層とよぶ．
...
\chapter{特性多様体}\label{chap2}
\chapter{\(D\)加群の構成法}\label{chap3}
\chapter{\(D\)加群の関手的性質}\label{chap4}
\chapter{確定特異点型ホロノミー系}\label{chap5}
