...
\subsection*{フーリエ・佐藤変換の超局所台}

\(F\in\Dbconic(E)\)に対し，
\[
    F^{\wedge}\coloneqq
    \Rder{{p_{2}}_{\ast}}\RG_{P}(p_1^{-1}F)
\]
と定めた．ただし\(p_1\)と\(p_2\)はそれぞれ
\(E\mathbin{\times_{Z}}E^{\ast}\)から，第1・第2成分への射影であり，
\[
    P=\left\{
        (z,x,y)\in E\mathbin{\times_{Z}}E^{\ast}
        \mathrel{;}
...
\begin{proof}    
    \(F^{\wedge\wedge}=F\)なので，
    \(\muS(F^{\wedge})\subset\muS(F)\)
    を示せば，
    \[
        \muS(F)
        =\muS(F^{\wedge\wedge})
        \subset\muS(F^{\wedge\wedge\wedge})
        =\muS(F^{\wedge})
    \]も従う．
    \(\cotb[E]\)の点を局所座標を用いて
    \(
        (z_{\circ},x_{\circ};\zeta_{\circ},\xi_{\circ})
    \)と表し，\(
        (z_{\circ},x_{\circ};\zeta_{\circ},\xi_{\circ})
        \notin 
        \muS(F)
    ...
    このとき，
    \(
        (z_{\circ},\xi_{\circ};\zeta_{\circ},-x_{\circ})
        \notin 
        \muS(F^{\wedge})
    \)を示せばよい．3段階に分けて示す．
    \subparagraph{Step~1}%\label{prf:555-1} 
    まず，
    \begin{align}
        x_{\circ}\ne0,
        \quad
    ...
    \end{align}
    の2つを仮定して考える．次の記号を準備する．
    \begin{itemize}
        \item \(j\colon \dot{E}\hookrightarrow E\)を閉埋め込みとする．
        \item \(\tilde{j}\colon \dot{E}\mathbin{\times_{Z}} E^{\ast}\hookrightarrow E\mathbin{\times_{Z}} E^{\ast}\)を\(j\)から定まる閉埋め込みとする．
        \item \(\dot{p}_{2}\colon \dot{E}\mathbin{\times_{Z}} E^{\ast}\to E^{\ast}\)を射影とする．
    \end{itemize}
    \eqref{eq:5513}より，
    \begin{align*}
        \RG_{P}(p_1^{-1}F)
        &\cong \RG_{P}(p_1^{-1}\Rder{j_{\ast}}j^{-1}F)\\
        &\cong \RG_{P}(\Rder{\tilde{j}_{\ast}}\tilde{j}^{-1}p_1^{-1}F)\\
        &\cong \Rder{\tilde{j}_{\ast}}\tilde{j}^{-1}\RG_{P}(p_1^{-1}F)
    \end{align*}
    が成り立つ．
    したがって，\[
        F^{\wedge}
        =
        \Rder{{\dot{p_{2}}_{\ast}}}\tilde{j}^{-1}\RG_{P}(p_1^{-1}F)    
    \]
    となる．
    %\begin{redleftbar}
    \begin{CMT*}
        \begin{enumerate}[(i)]
            \item 
            1つ目の同型は，\eqref{eq:5513}から従う．
            \item         
            2つ目の同型は，
            \(j\)と\(\tilde{j}\)がどちらも固有射であることから，
            カルテシアン図式
            \[\vcenter{\xymatrix@C=36pt@R=36pt{
                ...
                &
                E\pb[Z] E^{\ast}
                \ar[d]^-{\mres[\dot{p}_{1}]{E\pb[Z] E^{\ast}}}\\
                \dot{E}
                \ar@{^{(}->}[r]^-{j}
                &
                E
            ...
            固有基底変換を用いて
            \begin{align*}
                p_1^{-1}\Rder{j_{\ast}}j^{-1}F
                &\underset{\text{B.C.}}{\cong} 
                \Rder{\tilde{j}_{\ast}}(\mres[p_1]{\dot{E}\mathbin{\times_{Z}} E^{\ast}})^{-1}j^{-1}F\\
                &\underset{\text{可換図式}}{\cong} 
                \Rder{\tilde{j}_{\ast}}\tilde{j}^{-1}p_1^{-1}F
            \end{align*}
            のように示される．
            \item     
            3つ目の同型は，
            局所コホモロジー関手\(\RG_{P}\)と
            \(
                \Rder{\tilde{j}_{\ast}}\tilde{j}^{-1}
                =
                \RG_{\dot{E}\mathbin{\times_{Z}} E^{\ast}}
            \)が交換することから従う．
        \end{enumerate}
    \end{CMT*}
    %\end{redleftbar}
    ...
    \(
        ...
        \muS(F^{\wedge})
    \)だったとしてムジュンを導く．
    \(p_2\)に\cite[Prop.5.5.4.(iii)]{KS90}を用いると，
    \[
        \muS(\Rder{\dot{p_{2}}_{\ast}}\tilde{j}^{-1}\RG_{P}(p_{1}^{-1}F))
        \subset
        \mpb[\dot{p_{2}}]\mdiff[\dot{p_{2}}]^{-1}\muS(\tilde{j}^{-1}\RG_{P}(p_{1}^{-1}F)),
    \]
    すなわち，仮定から，\(x_1\in \dot{E}\)で
    \[
        (z_{\circ},x_1,\xi_{\circ};\zeta_{\circ},0,-x_{\circ})
        \in 
        \tilde{j}^{-1}\RG_{P}(p_{1}^{-1}F)
        =
        \mres[\RG_{P}(p_{1}^{-1}F)]{\dot{E}\mathbin{\times_{Z}} E^{\ast}}
    \]
    となるものが存在する\footnote{
        \(x_1\in \dot{E}\)となるのは，\(\tilde{j}\)で制限しているからで，
        \(x_1\)のファイバーが\(0\)になるのは，\(\mpb[\dot{p_{2}}]\mdiff[\dot{p_{2}}]^{-1}\)の部分から来ている．
    }．

    \subparagraph{Step~2}%\label{prf:555-2} 
    次に\eqref{eq:5513}を取り除き，\eqref{eq:5512}のみ仮定する．
    完全三角\[
        \RG_{Z}(F)\to F \to \Rder{j_{\ast}}j^{-1}F\to+1
    \]を考える．\(\Rder{j_{\ast}}j^{-1}F\)は
    \eqref{eq:5513}をみたすので，Step~1の結果から，
    \((z_{\circ},\xi_{\circ};\zeta_{\circ},-x_{\circ})
    \notin
    \muS((\Rder{j_{\ast}}j^{-1}F)^{\wedge})\)である．
    三角不等式\[
        \muS(F^{\wedge})            
        \subset
        \muS(\RG_{Z}(F)^{\wedge})            
        \cup
        \muS((\Rder{j_{\ast}}j^{-1}F)^{\wedge})
    \]より，\(
        (z_{\circ},\xi_{\circ};\zeta_{\circ},-x_{\circ})
        \notin
        \muS(\RG_{Z}(F)^{\wedge})
    ...
    \(
        ...
        \notin
        \muS(\RG_{Z}(F)^{\wedge})
    \)を示すために，\(i\colon Z\hookrightarrow E\)を閉埋め込みとする．
    次の図式を考える．
    \begin{align*}
        \vcenter{\xymatrix{
        Z\times\left\{0\right\}
    ...
    \vcenter{\xymatrix{
        Z\times\left\{0\right\}^{\ast}
        \ar@{^{(}->}[r]_-{i_{\tau}}
        \ar[d]
        &
        E\times\left\{0\right\}^{\ast}
        \ar[d]\\
        Z
        \ar@{^{(}->}[r]_-{i}
        &
    ...
    \end{align*}
    このとき，\cite[Prop.3.7.13,(i)]{KS90}と
    \cite[Prop.3.7.14,(i)]{KS90}から，
    \begin{align*}
        (i_{!}i^{!}F)^{\wedge}
        &\cong
        {\mtp[i]}^{-1}((i^{!}F)^{\wedge})\\
    ...
    \end{align*}
    となる．
    よって，\((z,y;\zeta,\eta)\in\cotb[E^{\ast}]\)に対し\(
        \pi^{-1}i^{!}F
    \)の茎の同型\[
        (\pi^{-1}i^{!}F)_{(z,y;\zeta,\eta)}
        \cong
        (i^{!}F)_{(z,y)}
    \]が成り立つことと\cite[Prop.5.4.5 (ii)]{KS90}から，
    \[
        \muS(\pi^{-1}i^{!}F)
        \subset
        \left\{(z,y;\zeta,\eta)\mathrel{;}\eta=0\right\}
    \]
    となるので，
    特に\(
        (z_{\circ},\xi_{\circ};\zeta_{\circ},-x_{\circ})
        ...
        \muS(\pi^{-1}i^{!}F)
    \)である．
    \subparagraph{Step~3}%\label{prf:555-3} 
    最後に\eqref{eq:5512}も取り除いた一般の場合を考える．

    \(\rr^{2}\)上の層\(A_{\rr\times\left\{0\right\}}\)を考える．
    \begin{CMT*}
        \begin{itemize}
            \item \(E\times \rr^2\)は\(Z\)上のベクトル束になる
            と思う．ねじれとかの議論は要らないのか．
            \item このあとの，
            \(F\etens A_{\rr\times\left\{0\right\}}\)は
            \(\Dbconic(E\times \rr^2)\)の対象となる．
            したがって，そのフーリエ・佐藤変換は
            \(\Dbconic(E^{\ast}\times (\rr^2)^{\ast})\)の
            対象となる．
        \end{itemize}        
    \end{CMT*}
    \cite[Prop.3.7.15]{KS90}から
    \begin{equation}
        (F\etens A_{\rr\times\left\{0\right\}})^{\wedge}
        \cong
        F^{\wedge}\etens A_{\left\{0\right\}\times\rr}[-1]
    \end{equation}
    である．
