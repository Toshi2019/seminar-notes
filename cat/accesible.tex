...
\section{\(\kappa\)フィルター余極限{\cite[sect.1.1]{MP89}}}
\begin{leftbar}
\begin{DFN}[{\cite[def.1.1.1]{MP89}}]
    \(\kappa\)を無限正則基数とする．
    圏\(I\)が\(\kappa\)フィルター (\(\kappa\)-filtered) であるとは，
    \(\kappa\)より小さな濃度をもつ任意のグラフ\(G\)に対して，
    任意の図式\(D\colon G\to I\)が余錐をもつ，すなわち
    図式\(\bar{D}\colon G^{+}\to I\)で\(D\)に拡張される図式
    をもつことをいう．
\end{DFN}
\end{leftbar}
...
\begin{PRP}
    ...
    次の条件は同値である．
    \begin{enumerate}[(i)]
        \item 次の2つの条件が成り立つ．\begin{enumerate}[(a)]
            \item 任意の\(A\subset\Ob(I)\)で\(\card(A)<\pi\)をみたすものに対し，\(j\in I\)でどの\(a\in A\)に対しても\(I\)の射\(a\to j\)が存在するものが存在する．
            \item 任意の\(i,j\in I\)と任意の\(B\subset\Hom_{I}(i,j)\)で\(\card(B)<\pi\)となるものに対し，\(I\)の射\(j\to k\)で合成\(i\overset{s}{\to}j\to k\)が\(s\in B\)によらないものが存在する．
        \end{enumerate}
        \item 任意の圏\(J\)で\(\card(\Mor(J))<\pi\)となるものと任意の関手\(\varphi\colon J\to I\)に対し，\(i\in I\)で\(\indlim[j\in I]\Hom_{I}(\varphi(j),i)\ne\varnothing\)となるものが存在する．
    \end{enumerate}
\end{PRP}
\end{leftbar}
%===============================================
...
\begin{thebibliography}{20} 
  ...
  \bibitem[MP89]{MP89} Michael Makkai, Robert Par\'e, 
    \textit{Accesible Categories: The Foundation of Categorical Model Theory}, 
    AMS, 1989.
\end{thebibliography}
