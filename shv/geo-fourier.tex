...
\title{幾何学的フーリエ変換 I}
\author{ブリリンスキー・マルグランジュ・ヴェルディエ}
\date{}
\begin{document}
\maketitle

\begin{abstract}
    ベクトル束上の層の複体を双対束上の複体への変換を定義し，
    それについて調べる．
    この変換はフーリエ変換の幾何学的類似である．
\end{abstract}

\section{記号}
本稿では\(A\)は可換なネーター環を表し，
\(Y\)はパラコンパクトかつ有限次元の局所コンパクト空間を表す．
\(E\overset{\pi}{\to}Y\)を\(Y\)上の階数が一様に\(r\)である
実ベクトル束とし，
\(E'\overset{\pi'}{\to}Y\)を\(E\)の双対ベクトル束とする．

\(B\) (resp. \(B'\))を\(E\) (resp. \(E'\)) の球面化とする．
\(B\)の\textbf{光線} (rayons) とは集合\(
    \big{\{}
        \left]0,+\infty\right]x\mid x\in E\setminus Y
    \big{\}}
\)のことである\footnote{
    Kashiwara-Schapira でいう\(x\in E\setminus Y\)の\(\rr^+\)軌道．
}．
\(B\) (resp. \(E\)) 上の\(A\)加群の層が\textbf{均質}\footnote{
    Kashiwara-Schapira でいう錐状 (conic) sheaf のことか．
} (homog\'ene) であるとは，
\(B\)の（\(E\)の）光線上で定数層であることをいう．
さらに，複体\(G\in\Domp(E,A)\)が均質 (homog\'ene) であることを
各\(H^iG\)が均質であることとして定める．
\(\Domp_{\mathrm{hom}}(B,A)\)で均質複体からなる\(\Domp(B,A)\)の部分圏を表す．



\section{幾何学的フーリエ変換の定義}
