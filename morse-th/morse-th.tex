...
\maketitle
\begin{abstract}
Morse理論の勉強ノート．実質\cite{M01}の読書ノート．
\end{abstract}
...
\begin{Example}
    ...
    よって$x=0$は$x^2$の臨界点である．

    2.
    $f(x)=x^3$とおく．
    $f'(x)=3x^2$なので，$f'(0)=0$である．
    よって$x=0$は$x^3$の臨界点である．
\end{Example}

$u<v$を実数とし，
$y=f(x)$を開区間$(u,v)$で定義された$C^{\infty}$級関数とし，
$(u,v)$の点$a$を$y=f(x)$の臨界点とする．
このとき，臨界点$x=a$が退化しているとは
\begin{align}
    f''(a)=0
\end{align}
であることをいう．
$x=a$が退化していないとき，非退化な臨界点であるという．

\subsection{2変数}

2変数のときの臨界点は
\begin{align}
    \frac{\p f}{\p x}(a,b)=\frac{\p f}{\p y}(a,b)=0
\end{align}
で定義する．

\begin{DFN}
    $(x,y)=p_0=(a,b)$を$f$の臨界点とする．
    $f$のヘッセ行列
    \begin{align}
        H_f(p_0)=
        \begin{bmatrix*}
            \dip \frac{\p^2 f}{\p x^2}(p_0)   &\dip\frac{\p^2 f}{\p y\p x}(p_0)\\\\
            \dip \frac{\p^2 f}{\p x\p y}(p_0) &\dip \frac{\p^2 f}{\p y^2}(p_0)\\
        \end{bmatrix*}
    \end{align}
    の行列式（ヘッシアン）$\det H_f(p_0)$が0のとき，$p_0$は退化している
    という．
\end{DFN}
