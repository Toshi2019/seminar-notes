...
\begin{THM}[{\cite[5.4節]{KS90}}参照]
    \(f\colon M\to N\)を多様体の射とし，
    \(F\in\Dompb(\kk_M)\)，\(G\in\Dompb(\kk_N)\)とする．
    \begin{enumerate}[(i)]
        \item \(f\)が\(\supp(F)\)上固有とする．このとき\(\muS(\Rder{f_{!}})\subset\mpb[f]\mdiff[f]^{-1}\muS(F)\).
        \item \(f\)が\(\muS(G)\)上非特性的であるとする．このとき\(\muS(f^{-1}G)\subset\mdiff[f]\mpb[f]\muS(G)\).
        \item \(f\)が平滑であるとする．
        このとき\(\muS(F)\times M\mathbin{\times_{N}} \cotb[N]\)となるのは，
        どの\(j\in\zz\)に対しても，層\(H^j(F)\)が\(f\)のファイバー上で局所定数層となるときである．
    \end{enumerate}
\end{THM}
\begin{CRL}
...
\begin{CRL}
    ...
    \(F\in\Dompb(\kk_{M\times I})\)が\(
        \muS(F)\cap \cotz[M]\times\cotb[I]\subset\cotz[M\times I]
    \)をみたし\(q\)が\(\supp(F)\)上固有とする．
