...
% documentclass
% ------------------------
\documentclass[9pt, b5paper, dvipdfmx, leqno, draft]{jsarticle}
...
%\layout
\section*{はじめに}
\cite{JS16,KS79}にしたがって，法錐 (normal cone) についてまとめる．
\section*{記号}
次の記号は断りなく使う．
\begin{itemize}
    \item 添字：
...
%\mainmatter
\section{法錐}
\(X\)を多様体，\(M\)を\(X\)の部分多様体，\(S\)を\(X\)の部分集合とする．
\(x\in X\)における\(S\)に対する\(M\)に沿った法錐\(C_{x}(S,M)\)は
次のようにかける．
\[
    v\in C_{x}(S,M)\Leftrightarrow
    \begin{cases*}
        \text{点列\(((x_n,y_n,c_n))_{n}\in(S\times M\times \rp)^{\nn}\)で}\\
        x_n\to x,\ y_n\to x,\ c_n(x_n-y_n)\to v\\
        \text{となるものが存在する．}
    \end{cases*}
\]

\(X\)がベクトル空間の開集合であるとき，
\(v\in T_{x}X\)が\(C_{x}(S,M)\)に属さないのは，
\(x\)の開近傍\(U\)と\(v\)を含む開錐\(\gamma\)で
\[
    ((U\cap M)+\gamma)\cap(U\cap S)=\varnothing
\]
となるものが存在するときである．
...
\section{狭義の法錐}
\(X\)を多様体，\(M\)を\(X\)の部分多様体，\(S\)を\(X\)の部分集合とする．
\(x\in X\)における\(S\)に対する狭義の法錐\(N_{x}(S)\)は
\[
    N_{x}(S)\coloneqq
    T_{x}X-C_{x}(X-S,S)
\]
である．さらに
\begin{align*}
    N_{x}^{\ast}(S)&=N_{x}(S)^{\circ},\\
    N(S)&=\bigcup_{x\in X}N_{x}(S),\\
    N^{\ast}(S)&=\bigcup_{x\in X}N_{x}^{\ast}(S)
\end{align*}
とおく．\(N(S)\)を\(S\)に対する\textbf{狭義の法錐} (strict normal cone)，
\(N^{\ast}(S)\)を\(S\)に対する\textbf{余法錐} (conormal cone) とよぶ．
\begin{align*}
    N_{x}(S)&=
    T_{x}X-C_{x}(X-S,S),
    &
...
\begin{thebibliography}{20} 
    ...
    \bibitem[JS16]{JS16} Benoit Jubin, Pierre Schapira, 
        \textit{Sheaves and D-modules on Lorenzian Manifolds}, 2016.
    \bibitem[KS79]{KS79} Masaki Kashiwara, Pierre Schapira, 
        \textit{Micro-Hyperbolic Systems}, 
        1979.
