...
\subsection{セミナー科目での学習内容}

研究室のセミナーでは
\cite{KS90}を第1章から読み始め，
\today 現在，第5章まで読み通した．
その主な内容は，以下の通りである．
\begin{itemize}
    \item 導来圏を用いたホモロジー代数
    \item 位相空間上の層の導来圏とGrothendieckの六則演算
    \item 層の特殊化・フーリエ佐藤変換・超局所化・\(\muhom\)関手
    \item 層の超局所台
\end{itemize}

数学独立探究では，
\cite{MS17}の第2章と第3章の一部を読んだ．
その主な内容は，以下の通りである．
\begin{itemize}
    \item アフィン空間における，シンプレクティック微分同相群・グロモフの圧縮不能性定理・概複素構造
    \item シンプレクティック多様体の定義と例
    \item ハミルトンイソトピー
    \item 接触構造
\end{itemize}
...
以下のものがある．
\begin{itemize}
  \item 位相的\(K\)理論の基本的な事柄（\cite[1,2章]{Ati22}等）
  \item \(D\)加群の基本概念，たとえば連接\(\cD_X\)加群の特性多様体やCauchy--Kovalevskaya--Kashiwaraの定理など（\cite{Kas08}等）
  \item シンプレクティック幾何の基本的な部分，たとえばシンプレクティック微分同相写像・Hamilton微分同相写像・Lagrange部分多様体など（\cite{HZ94}等）
\end{itemize}
\subsection{学外での学習内容}

その他，学外の研究集会・ワークショップ・集中講義で出席したものを以下に挙げる．
\begin{itemize}
    \item 層量子化と深谷圏：九州大学 2023年度 数理科学特別講義 VII 2023.10.2(月) -- 2023.10.6(金)
    \item 超局所解析と漸近解析における諸問題：数理解析研究所 2023年11月6日（月）～11月10日（金）
    \item Aubry-Mather理論とその周辺：2024年度 力学系勉強会 2024年9月9日(月)〜9月12日(木)
    \item Prospects in microlocal analysis and asymptotic analysis: 数理解析研究所 7 (Mon.) - 11 (Fri.), October 2024
    \item Recent topics in algebraic analysis：日本大学 2025年3月6日(木) -- 3月8日(土)
\end{itemize}

\section{研究内容}

\today 現在，投稿した論文は無い．
%\section{北海道大学の本専攻を選んだ動機}
