...
左導来関手\(\blk\ltens[\scR]\blk\)を定義するためには，
\(\scR\)に対する仮定が必要である．

\begin{DFN}
    \(\scR\)を\(X\)上の環の層とする．\[
        \wgld(\scR)\coloneqq\sup_{x\in X}\left(
            \wgld(\scR_x)
        \right)
    \]とおく（演習\ref{EXC129}）．
    \(\wgld(\scR)\)を\(\scR\)の弱大域次元とよぶ．
\end{DFN}

環の層\(\scR\)上のテンソル積を考える際には，
常に\(\scR\)の弱大域次元は有限であると仮定する：
\begin{equation}
    \wgld(\scR)<\infty.
\end{equation}
命題\ref{PRP2412}，系\ref{CRL178}，演習I.23 を適用することで，
\(F\in\Dompb(\scR)\)または\(F\in\Dompl(\scR)\)ならば，
\(F\)は平坦\(\scR\)加群の有界複体，または下に有界な複体と
それぞれ擬同形であることが分かる．
したがって左導来関手
\begin{align*}
    \blk\lltens[\scR]\blk&\colon\Domp^{\ast}(\scR^{\op})\times\Domp^{\ast}(\scR)\to\Domp^{\ast}(\scS),\\
    \blk\lltens[\scS]\blk&\colon\Domp^{\ast}(\scS)\times\Domp^{\ast}(\scS)\to\Domp^{\ast}(\scR)
\end{align*}
を\(\ast=-,+,\mathrm{b}\)に対して定義することができる．

とくに記法\ref{NTT2312}の状況を考える．
\(\ast=-,+,\mathrm{b}\)に対し関手\[
    \blk\letens[\scR]{S}\blk\colon
    \Domp^{\ast}(p_{X}^{-1}(\scR^{\op}))
    \times
    \Domp^{\ast}(p_{Y}^{-1}\scR)
    \to\Domp^{\ast}(p^{-1}\scS)
\]が定まる．（ここで\(\scR\)と\(\scS\)は\(S\)上の環の層である．）

\subsection*{導来関手の間の関係}

以上全ての関手の間には多くの関係があるが，
ここではその中のいくつかについて簡単に述べる．
命題1.8.7と1.10.9を組織的に用いる．

\(F\in\Dompl(\scR)\)とするとき，次が成り立つ．
\begin{equation}
    \RG_{Z}(X;F)\RG(X;\RG_Z(F)).
\end{equation}

\begin{proof}
    \(
        \cI(\scR)\coloneqq\left\{
            \text{\(\scR\)入射加群}
        \right\}
    \)とおくと，\(\cI(\scR)\)は\(\Gamma(X;\blk)\)と\(\Gamma_Z\)に関して入射的である．
    さらに\(I\)が入射的ならば\(\Gamma_Z(I)\)も入射的なので，命題1.8.7より
    \[
        \Rder(\Gamma_Z(X;\blk))
        \cong
        \Rder(\Gamma(X;\blk)\circ\Gamma_Z)
        \cong
        \RG(X;\blk)\circ\RG_Z
    \]である．
\end{proof}

\(F\in\Dompu(\scR)\)とし\(G\in\Dompl(\scR)\)とする\footnote{
...
\eqref{eq267}に\(\RG(X;\blk)\)を適用し，0次コホモロジーをとることで
次の同型が得られる．
\begin{align}
...
\end{PRP}
\end{leftbar}
\footnotetext[1]{\cite{KS90}では式番号 (4.1.6) は無い}
...
\begin{CMT}
    ...
    \]とおくとき，\begin{description}
        %\item \(U\in\cW\)ならば\(U\)は\(V\)の開近傍であり，
...
\(f\colon Y\to X\)を多様体の射とし，\(M\)を\(X\)の閉部分多様体とする．
...
\)は「\(X\times\Int\gamma^{\circ a}\)における同型」となる．
本章では，この超局所的な同型を定義し，次章で理論を展開する．
...
Kashiwara-Schapira[3]の説明にそっくり従うことが多いが，
証明のいくつかは単純化されている．
...
\begin{EG}
    \begin{figure}[htp]
        \centering
        \begin{tikzpicture}
            \draw[->,>=stealth,semithick] (-1,0)--(4,0)node[above]{$x$}; %x軸
            \draw[->,>=stealth,semithick] (0,-5)--(0,5)node[right]{$y$}; %y軸
            %\draw (0,0)node[above  left]{O}; %原点
            %\fill[gray!20!white] plot[domain=0:3](\x,{sqrt(\x^3)});
            \draw[very thick,samples=100,domain=0:3] plot({\x},{sqrt(\x^3)}); 
            \draw[very thick,dashed,samples=100,domain=0:3] plot({\x},{-sqrt(\x^3)}); 
...
\(f\)が\(Y\)から\(X\)への写像ならば，
第\ref{chap4}章のように，\(\mtp[f]\)と\(f_\pi\)で
以下の写像を表す．
\[
    \cotb[Y]
    \underset{\mtp[f]}{\longleftarrow}
    Y\pb[X]\cotb[X]
...
\chapter{\(\cO_X\)加群と\(\cD_X\)加群への応用}

\section*{付記}
連接\(\cO_X\)加群の歴史について詳細に論じるのは本書の範囲を超えるが，
Oka[1], Cartan[2], Serre[1] の名前を挙げない訳にはいかない．
概要についてはH\"ormander[1]を参照．

\(\cD_X\)加群についても同様に，復習はせず Schapira[2]内の付記を挙げ，
ここでは単にKashiwara[1]とBernstein[1,2]において，70年代に，
その理論が現れたことに言及しておく．

古典的なCauchy-Kowalewskiの定理はLeray[3]によって
精密化されThm11.3.1の形で定式化された．
その後2つの方向性で方程式系に拡張された．
1つはKashiwara[1]によるもので，「Cauchy問題」(Thm11.3.5) に関係する．
もう1つは「伝播」に関するものである．
これはKashiwara-Schapira[2,3]によるThm11.3.3で，
それ以前のZerner[1], Bony-Schapira[1], Kashiwara[5]による
部分的な結果の後に得られたものである．
Thm11.3.7はKashiwara[3]ですでに得られていた．
これは多くの重要な発展の契機となった．
それについてはBj\"ork[2]を挙げる．
