...
もうひとつ，\(X\)の閉部分集合\(Z\)に対して関手的に定まる層ががある．
...
\(V\)を\(Z\)を含む\(U\)の開集合とする．
自然な射\(\Gamma_Z(U;F)\to\Gamma_Z(V;F)\)は同型である．
したがって\(X\)の局所閉集合\(Z\)に対し，\(Z\)を
閉集合として含む\(X\)の任意の開集合\(U\)を用いることで，\(
    \Gamma_Z(X;F)
\)を\(\Gamma_Z(U;F)\)として定めることができる．
前層\(U\mapsto\Gamma_{Z\cap U}(U;F)\)は層である．
