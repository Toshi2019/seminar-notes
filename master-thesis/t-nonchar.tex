...
\title{Truncated Noncharacteristic Deformation Lemma}
...
\section{Noncharacteristic Deformation Lemma}
  ...
  \begin{figure}
    ...
    \begin{minipage}[b]{0.32\columnwidth}
      ...
      \begin{tikzpicture}[scale=1.5]
        ...
        % 角度は辺の比に反比例する（弧の長さが辺に対応）
        % AB:BC:CA = 10:9:8 → 弧の比 = 10:9:8 → 角度の比 = 10:9:8

        % 合計角度 = 360度なので
        \pgfmathsetmacro{\angleA}{(10/27)*360}
        \pgfmathsetmacro{\angleB}{(9/27)*360}
        \pgfmathsetmacro{\angleC}{(8/27)*360}

        % 各点の位置（単位円上に配置）
        \coordinate (A) at ({\r*cos(0)}, {\r*sin(0)});
        \coordinate (B) at ({\r*cos(\angleC)}, {\r*sin(\angleC)});
        \coordinate (C) at ({\r*cos(\angleC+\angleB)}, {\r*sin(\angleC+\angleB)});

        % 円
        \draw[gray!60, dashed] (O) circle (\r);
    ...
    \begin{minipage}[b]{0.32\columnwidth}
      ...
      \end{tikzpicture}
    \end{minipage}
    \caption{Examples of deformation families.}
    \label{fig:1}
  \end{figure}
\end{Example+}
...
\begin{proof}
  ...
  \begin{proof}[Proof of Lemma\ref{clm:ind-nonchar}]
    ...
    \] 
    Since \(\left(U_{t}\right)_{t>s}\) is a 
    ...
    \]
    is isomorphic.     
    Since \(X\) is Hausdorff and \(Z_{s}\) is commpact, 
    the natural morhism 
    \[
    ...
    More precisely, there exists a family \(
      \left(\alpha^{U}\right)_{U}
      \in 
      \indlim[U\supset Z_{s}]\Gamma(U;F)
    \) such that \[
      \mres[\alpha^{U}]{Z_{s}}=\mres[\sigma]{Z_{s}}
    \] for all \(U\). 
