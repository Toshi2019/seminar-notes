...
\textcolor{darkgreen}{in the 1980's}.} 
...
derived category \textcolor{darkgreen}{from below}) of 
...
\textcolor{darkgreen}{some} properties 
of truncated microsupports were shown. 
For example, 
\textcolor{darkgreen}{a truncated microsupport 
...
to \textcolor{darkgreen}{the study of} some geometrical problems. 
First we} prove a truncated version of 
the noncharacteristic deformation lemma. 
\textcolor{darkgreen}
{Consider a Hausdorff space a family of open subsets of it. 
...
result\textcolor{darkgreen}{s} 
in microlocal sheaf theory, \textcolor{darkgreen}{and} 
...
\textcolor{darkgreen}{Second, we weaken the hypothesis of vanishing of 
...
\textcolor{darkgreen}{Third, we make an additional assumption on 
the family of deformation.}
Therefore, 
to make\textcolor{darkgreen}{, in particular, the third} 
...
microlocal \textcolor{darkgreen}{M}orse lemma (Proposition \ref{prp:mu-morse}), 
...
\begin{Proposition+}
  ...
  \end{equation}
  \textcolor{darkgreen}{Moreover if \(f\) is a closed embedding, 
...
\begin{Example+}\label{eg:t-nonchar}
  ...
  \begin{enumerate}[(i)]
    ...
    \[
      Z_{s}=\left\{
        (x,y)\in X;\ y=\varphi_{s}(x)\textcolor{darkred}{, \left|x\right|\leqq 2}
      \right\}
    ...
    \) for all \(\textcolor{darkred}{t \geqq s}\) and \(x\in Z_{s}- U_{t}\), 
    but the morphism \(
      H^{0}(U_{1};\kk_{Z})=\kk
      \to 
      0 = H^{0}(U_{0};\kk_{Z})
...
\begin{proof}[{\textcolor{black}{Proof of Theorem \ref{prp:t-nonchar}}}]
  ...
  \]
  \textcolor{darkgreen}{Therefore we}
  \[
    ...
    \text{for any \(j\leqq k\).}
  \]
  Thus we have \textcolor{darkgreen}{from \eqref{eq:t-nocha-d.t.}}
  \begin{align}
    \label{eq:a-iso}
  ...
  we have \textcolor{darkgreen}{from Lemma \ref{clm:ind-nonchar} and \eqref{eq:a-iso}}
  \begin{align*}
  ...
  Similarly, 
  we have \textcolor{darkgreen}{from Lemma \ref{clm:ind-nonchar} and \eqref{eq:a-inj}}
  \begin{align*}
...
\textcolor{darkgreen}{a truncated version of a particular case}
...
\begin{proof}
  ...
  and verify the hypotheses of the \textcolor{darkgreen}{theorem}.  
  \begin{enumerate}[(i)]
    ...
    and \[
      ...
      =\left\{s\right\}. 
    \]
    By the hypothesis \textcolor{darkgreen}{of the lemma}, 
    we have 
    \[
  ...
  \[
    ...
    H^{j}(I_{t};F)
  \]
  and the \textcolor{darkgreen}{monomorphisms}
  \[
...
\begin{proof}
  ...
  \]for each \(j\leqq k\). 
  Then we can apply Lemma \ref{Lem:1d-t-morse} and \textcolor{darkgreen}{obtain}
  \[
...
\begin{proof}[Proof of Lemma \ref{clm:ind-nonchar}]
  ...
  it follows from 
  \textcolor{darkgreen}{the assumption for contradiction}
  that
  \begin{align*}
  ...
  \textcolor{darkgreen}{Since \(\tilde{\phi}\) is an} isomorphism, 
  it is enough to show that
  \[
  ...
  \). 
  \textcolor{darkgreen}{On the other hand, 
