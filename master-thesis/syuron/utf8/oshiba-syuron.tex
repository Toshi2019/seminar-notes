...
%In this paper, 
%\textcolor{darkred}{w}e study microlocal Morse theory. 
%\textcolor{darkred}{This} is presented, 
%for instance, in \cite{GKS12}. 
\subsection{\textcolor{darkred}{Microlocal Sheaf Theory}}

\textcolor{darkred}{Microlocal Morse theory is a machinary 
...
\textcolor{darkred}{To explain more precisely, 
Let fix some notations. 
...
This notion can be interepreted by an analogy of Morse theory. 
Let \(\varphi\) be a real-valued function of class \(C^1\). 
Then we can replace the notion of a critical point of \(\varphi\) with 
one with respect to \(F\). 
That is to say, we can define a critical point 
with respect to \(F\) by the point \(p\) 
such that \(d\varphi(p)\) does not belongs to \(\muS[](F)\). 
We can then obtain a analogous result of Morse theory in a
framework of sheaf theory.  
The following is called (a special case of) 
...
\subsection{\textcolor{darkred}{Truncated Microsupport}}

%However, we replace the notion of microsupports with
\textcolor{darkred}{In 2003, 
Kashiwara, Monteiro-Fernandes, and Schapira
introduced the notion of truncatd microsupprt in \cite{KFS03}}, 
in course of the study of solutions of 
boundary value problems of \(\mcal{D}\)-modules. 
\textcolor{darkred}{The difference between 
...
all the degree of a certain cohomology vanish, 
whereas \(p\) does 
not belong to the truncated microsupport if and only if 
the same kind of cohomology of all the degree under 
a fixed integer vanish.
}
It was in the context of such kinds of analytic problems 
that they studied truncated microsupports. 

\subsection{\textcolor{darkred}{Results}}

\textcolor{darkred}{In this paper, 
in contrast to the previous research, 
we shall apply the truncated microsupport to study some geometrical problems. 
First we shall} prove a truncated version 
of the noncharacteristic deformation lemma\textcolor{darkred}{. 
...
\textcolor{darkred}{
This lemma is the most fundamental result in microlocal sheaf theory 
used all along \cite{KS90}. 
Thus the truncated version of this lemma 
is also the key result in this paper. 
However, one should notice that 
the original proof of the lemma cannot be applied 
to our truncatd version. 
To prove this result, 
we weaken a hypothesis of the lemma and 
make an additional assumption of 
...
\textcolor{darkred}{truncated version of} 
microlocal morse lemma, 
...
if it can be extended. \footnote{この辺は拡張をいろいろな言い方で言い換えていますが，同じ方がいいですかね？}

%In \cite{KFS03}, they do not study the functorial property of 
...
\subsection{\textcolor{darkred}{Organization}}
...
In this section, 
we recall some definitions and results \textcolor{darkred}{of 
...
Let us denote by \(Z\) a locally closed set of \(X\) 
and by \(i\colon Z\hookrightarrow X\) \textcolor{darkred}{the} inclusion. 
...
If \(f\colon M\to N\) is a morphism of manifolds \textcolor{darkred}{(i.e.\ a \(C^{\infty}\)-map)}, 
then we have the natural diagram. 
\[\vcenter{\xymatrix{
  TM
    \ar[r]^-{f'}
    \ar[rd]_-{\tau_M} 
...
the natural projections\textcolor{darkred}{,} \(f'\) is defined by 
...
\begin{Proposition+}[{\cite[Proposition 2.5.1]{KS90}}]\label{prop:ks2.5.1}
  ...
  \textcolor{darkred}{Then the following hold.}
  \begin{enumerate}[(i)]
...
\textcolor{darkred}{Note that a} paracomact space 
is by definition a Hausdorff space. 

\subsection{\textcolor{darkred}{Complement of Homological Algebra}}\label{ssec:ML}

Consider a projective system of abelian groups \(
  \left(M_n,\rho_{n,p}\right)_n
\) indexed by \(\nn\). 
One says that this system satisfies 
the Mittag-Leffler condition (ML \textcolor{darkred}{condition} for short) 
...
\end{equation}
and \textcolor{darkred}{its} projective limit
\begin{equation}
...
\begin{Proposition+}[{\cite[Proposition 1.12.4]{KS90}}]\label{prop:ks1.12.4}
  ...
  the system \(\left(M_{n}^{k-1}\right)_{n}\) satisfies 
  the ML condition. 
  Then
  \begin{enumerate}[(a)]
...
\subsection{\textcolor{darkred}{ML Procedure for Sheaves}}
...
\textcolor{darkred}{Before that we review the original definition of a microsupport.
\begin{Definition+}
  Let \(F\in\Dompb(\kk_X)\). 
...
\textcolor{darkred}{Kashiwara, Monteiro-Fernandes, and Schapira modified
this definition and gave the notion of a truncated microsupport.}
...
\textcolor{darkred}{Truncated microsupports satisfy 
...
\begin{Theorem+}[Truncated Noncharacteristic Deformation Lemma]\label{prp:t-nonchar}
  ...
  open subsets of \(X\). 
  Let \(k\in\zz\). 
  \textcolor{darkred}{For \(s\in\rr\)}, set \(
    Z_{s}\coloneqq \bigcap_{t>s}\overline{U_t\smi U_s}
  \). 
  We assume the following conditions. 
  \begin{enumerate}[(i)]
    ...
    For all pairs \((s,t)\) with \(s< t\), \(Z_s\subset U_{t}\).
  \end{enumerate}
  Then, for all \(t\in\rr\), we have the isomorphisms\[
...
\textcolor{darkred}{Before we prove Theorem \ref{prp:t-nonchar}, 
...
\begin{Example+}\label{eg:t-nonchar}
  ...
  \textcolor{darkred}{Theorem \ref{prp:t-nonchar}} is not contained 
  in the original version (\textcolor{darkred}{Proposition \ref{prp:nonchar}}). 
  ...
  \begin{enumerate}[(i)]
    ...
    \[
      Z_{s}=\left\{
        (x,y)\in X;\ y=\varphi_{s}(x)
      \right\}
    \] and the point \((2,0)\in Z_{s}\) is 
    not contained in \(U_{t}\). 
    Moreover, let us \textcolor{darkred}{define 
    a locally closed subset \(Z\) of \(X\) by}
    \[
      Z\coloneqq \left\{(x,y)\in X;\ (x+2)^{2}+(y-1)^{2}=1\right\}\setminus \left\{(-1,0)\right\}. 
    \]
    \textcolor{darkred}{For this \(Z\)} 
    we denote by \(\kk_{Z}\) the sheaf on \(X\) \textcolor{darkred}{as Section \ref{sec:review}}. 
    If \(j\leqq0\), we have \(
      H^{j}_{X\smallsetminus U_{t}}(\kk_{Z})_{x}=0
    \) for all \(t>s\) and \(x\in Z_{s}\smallsetminus U_{t}\), 
    but the morhism \(
      H^{0}(U_{1};\kk_{Z})=\kk
      \to 
      0 = H^{0}(U_{0};\kk_{Z})
    ...
    which result\textcolor{darkred}{s} in that 
  ...
  \begin{enumerate}[(i)]
    ...
    For a real number \(t\), 
    define the open set \(U_{t}\) \textcolor{darkred}{of \(X\)} 
    by \[
      U_{t}\coloneqq
      \begin{cases}
      \left\{(x,y)\in X;\ \left\|(x,y)\right\|<t\right\}&(t>0),\\
      \varnothing&(t\leqq0).         
      \end{cases}
    \]
    \begin{figure}[htb]
    ...
    If \(t> 0\), set \[
      \varphi_{t}(x)\coloneqq\begin{cases}
        \sqrt{t^{2}-x^{2}} & (\left|x\right|<s),\\
        0&(\left|x\right|\geqq s).
      \end{cases}
    \]
    Then define the open set \(U_{t}\) 
    \textcolor{darkred}{of \(X\)} by \[
      U_{t}\coloneqq
...
\begin{proof}[{\textcolor{darkred}{Proof of Theorem \ref{prp:t-nonchar}}}]
  ...
  \begin{proof}[Proof of Lemma \ref{clm:ind-nonchar}]
    ...
    Let \(\sigma\in \Gamma(Z_{s};F)\) be 
    a section of \(\mres[F]{Z_{s}\cup U_{s}}\). 
    ...
    \] 
    Thus we have \(W= ((Z_{s}\cup U_{s}))\cap \widetilde{W}\). 
    For this \(\widetilde{W}\), 
    the pair \(
      \left(Z_{s}\cup U_{s}, \widetilde{W}\right)
    \) is an open covering of \(\widetilde{W}\cup U_{s}\). 
    Thus by \eqref{eq:clm-glue}, we can 
    glue \(\sigma\) and \(\mres[\tau]{\widetilde{W}}\) together 
  ...
  \textcolor{darkred}{Theorem \ref{prp:t-nonchar}}. 
  ...
  \textcolor{darkred}{(Proposition \ref{prop:ks1.12.6})} that
  \[
...
\begin{proof}
  We will use \textcolor{darkred}{Theorem \ref{prp:t-nonchar}}. 
  ...
  \end{enumerate}
  Then we can apply \textcolor{darkred}{Theorem \ref{prp:t-nonchar}}, and 
...
\begin{Theorem+}\label{thm:t-morse}
  ...
  \). For \(t\in \rr\), we set \(
    M_t\coloneqq\psi^{-1}\left((-\infty,t)\right)
  \). 
  Then, the restriction \textcolor{darkred}{morphism} \[
    H^{j}(M_{b};F)\to H^{j}(M_{a};F)
  \]
  \textcolor{darkred}{is} isomorphic for each \(j< k\), and 
  injective for \(j=k\).
\end{Theorem+}
...
We would like to thank Naofumi Honda, our superviser for 
patient guidance. 
We also thank Ryosuke Sakamoto, 
Kango Ito, Yoshiaki Yamamura for fruitiful discussions. 
In particular, Sakamoto-san gave us several ideas 
