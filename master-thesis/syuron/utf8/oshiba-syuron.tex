\documentclass[a4paper,leqno,dvipdfmx]{jsarticle}
\usepackage{syuron2}
...
\textcolor{black}{Microlocal Morse theory is a machinery 
...
\textcolor{black}{in the 1980's}.} 

\textcolor{black}{To explain more precisely, 
\textcolor{black}{let us} fix some notations. 
...
derived category \textcolor{black}{from below}) of 
...
introduced the notion of truncated microsupport in \cite{KFS03a}}, 
...
\textcolor{black}{some} properties 
of truncated microsupports were shown. 
For example, 
\textcolor{black}{a truncated microsupport 
does not satisfy the involutivity, but
...
studied by several authors. See \cite{KFS03a,FM07}. 
...
to \textcolor{black}{the study of} some geometrical problems. 
...
the global section on a member of the family is 
...
result\textcolor{black}{s} 
in microlocal sheaf theory, \textcolor{black}{and} 
...
To prove \textcolor{black}{the} result, 
we \textcolor{black}{make three modifications 
to the lemma. 
...
\textcolor{black}{Second, we weaken the hypothesis of vanishing of 
...
\textcolor{black}{Third, we make an additional assumption on 
the family of deformation.}
Therefore, 
to make\textcolor{black}{, in particular, the third} 
...
microlocal \textcolor{black}{M}orse lemma (Proposition \ref{prp:mu-morse}), 
...
Let us denote by \(i\colon Z\hookrightarrow X\) \textcolor{black}{the} inclusion. 
...
We denote by \(\tau_{M}\colon TM\to M\) 
the tangent bundle of \(M\), 
and by \(\pi_{M}\colon \cotb[M] \to M\) 
the cotangent bundle of \(M\). 
If there is no risk of confusion, 
we will write \(\pi\) rather than \(\pi_{M}\). 
If \(f\colon M\to N\) is a morphism of manifolds 
\textcolor{black}{(i.e., a \(C^{\infty}\)-map)}, 
then we have the natural diagram\textcolor{black}{s}. 
\[\vcenter{\xymatrix{
  TM
    \ar[r]^-{f'}
    \ar[rd]_-{\tau_M} 
...
In \textcolor{black}{these diagrams}, 
...
\begin{Proposition+}[{\cite[Proposition 1.12.4]{KS90}}]\label{prop:ks1.12.4}
  ...
  the system \(\left(M_{n}^{\textcolor{black}{k}}\right)_{n}\) satisfies 
  the ML condition. 
  Then
  \begin{enumerate}[(a)]
...
\begin{Definition+}[{\cite[Definition 3.4]{KFS03a}}]
  Let \(F\in\Dompb(\kk_X)\)\textcolor{black}{, and fix an 
  integer \(k\in\zz\)}. 
  \textcolor{black}{The \emph{truncated microsupport} \(\muS[k](F)\) is} 
  the closed conic subset of \(\cotb[X]\) 
  defined by \textcolor{black}{the following condition}: 
  \(p\notin \muS[k](F)\) 
  if and only if 
...
\begin{Proposition+}
  ...
  \end{equation}
  \textcolor{black}{Moreover if \(f\) is a closed embedding, 
  the inclusion above is an equality.}
\end{Proposition+}
...
we postpone the proof to Section \ref{sec:proof-lem}. 
...
\begin{Theorem+}[Truncated Noncharacteristic Deformation Lemma]\label{prp:t-nonchar}
  ...
  \begin{enumerate}[(i)]
    ...
    For all pairs \((s,t)\) with \(s< t\), \textcolor{black}{we have} \(Z_s\subset U_{t}\).
  \end{enumerate}
  Then, for all \(t\in\rr\), we have the isomorphisms\[
  ...
  \]
  and the \textcolor{black}{monomorphisms} \[
...
\begin{Example+}\label{eg:t-nonchar}
  ...
  Therefore let us give an example of \(\left(U_{t}\right)_{t\in\rr}\) 
  ...
  \begin{enumerate}[(i)]
    ...
    Indeed, if \(\textcolor{black}{0<s<t}\), 
    we have 
    \[
      Z_{s}=\left\{
        (x,y)\in X;\ y=\varphi_{s}(x)\textcolor{black}{, \left|x\right|\leqq 2}
      \right\}
    ...
    \[
      Z\coloneqq \left\{(x,y)\in X;\ (x+2)^{2}+(y-1)^{2}=1\right\}- \left\{(-1,0)\right\}. 
    \]
    For this \(Z\) 
    we denote by \(\kk_{Z}\) the sheaf on \(X\) as \textcolor{black}{in} Section \ref{sec:review}. 
    If \(j\leqq0\), we have \(
      H^{j}_{X- U_{t}}(\kk_{Z})_{x}=0
    \) for all \(\textcolor{black}{t \geqq s}\) and \(x\in Z_{s}- U_{t}\), 
    but the morphism \(
      H^{0}(U_{1};\kk_{Z})=\kk
      \to 
      0 = H^{0}(U_{0};\kk_{Z})
    \) is not injective. 
    This gives an example that 
  ...
  \begin{enumerate}[(i)]
    ...
    by \[
      U_{t}\coloneqq
      \begin{cases}
      \left\{(x,y)\in X;\ \textcolor{black}{x^{2}+y^{2}} <t\right\}&(t>0),\\
      \varnothing&(t\leqq0).         
      \end{cases}
    \]
    \begin{figure}[htb]
...
\begin{proof}[{\textcolor{black}{Proof of Theorem \ref{prp:t-nonchar}}}]
  ...
  \begin{equation}\label{eq:t-nocha-d.t.}
    \RG(Z_{s}\cup U_{s};\mres[\RG_{X\smi U_{s}}(F)]{Z_{s}\cup U_{s}})
    \to
  ...
  \]
  Therefore we \textcolor{darkred}{have}
  \[
    ...
    \text{for any \(j\leqq k\).}
  \]
  Thus we have \textcolor{black}{from \eqref{eq:t-nocha-d.t.}}
  \begin{align}
    \label{eq:a-iso}
    ...
    \text{for any \(j< k\),}\\
    \label{eq:a-inj}
    H^{k}(Z_{s}\cup U_{s};\mres[F]{Z_{s}\cup U_{s}})
    &\hookrightarrow
    H^{k}(U_{s};F).
  \end{align}
  Therefore, if we show Lemma \ref{clm:ind-nonchar} below, 
  \textcolor{black}{whose proof will be given in Section \ref{sec:proof-lem},}
  we have \textcolor{black}{from Lemma \ref{clm:ind-nonchar} and \eqref{eq:a-iso}}
  \begin{align*}
  ...
  Similarly, 
  we have \textcolor{black}{from Lemma \ref{clm:ind-nonchar} and \eqref{eq:a-inj}}
  \begin{align*}
...
\textcolor{black}{a truncated version of a particular case}
...
\begin{proof}
  ...
  and verify the hypotheses of the \textcolor{black}{theorem}.  
  \begin{enumerate}[(i)]
    ...
    and \[
      ...
      =\left\{s\right\}. 
    \]
    By the hypothesis \textcolor{black}{of the lemma}, 
    we have 
    \[
...
\begin{Proposition+}[{\cite[Proposition 1.8]{GKS12}}]\label{prp:mu-morse}
...
\begin{Theorem+}\label{thm:t-morse}
  ...
  class \(C^1\), 
  proper on \(\supp(F)\). 
  Let \(a<b\) be real numbers, and \(k\in\zz\).
  We assume \(d\psi(x)\notin \muS[k](F)\) for \(
    a\leqq \psi(x)<b
  \). For \(t\in \rr\), we set \(
    M_t\coloneqq\psi^{-1}\left((-\infty,t)\right)
...
\begin{proof}
  ...
  \]for each \(j\leqq k\). 
  Then we can apply Lemma \ref{Lem:1d-t-morse} and \textcolor{black}{obtain}
  \[
...
\begin{proof}[Proof of Lemma \ref{clm:ind-nonchar}]
  ...
  \[\vcenter{\xymatrix{
    ...
    \ar[r]_-{\psi}^-{\sim}
    &
    \Gamma(Z_{s};\mres[F]{Z_{s}})\textcolor{black}{.}
  }}
  \] 
  ...
  \) of \(\overline{U_{t}- U_{s}}\) by \(
    A_{r}\coloneqq U_{r}- (U\cup U_{s})
  \). 
  This \(\left(A_{r}\right)_{s<r<t}\) satisfies 
  the finite intersection property. 
  Indeed, for finite real numbers \(s<r_{0}<\dots<r_{m}<t\), 
  it follows from 
  \textcolor{black}{the assumption for contradiction}
  that
  \begin{align*}
    ...
    &\ne \varnothing.
  \end{align*}
  Therefore, since \(\overline{U_{t}- U_{s}}\) is 
  ...
  %\begin{align*}
  %  \overline{A_{r}}
  %  &=\overline{U_{r}-(U\cup U_{s})}\\
  %  &=\overline{U_{r}}\cap 
  %  \overline{X -(U\cup U_{s})}\\
  %  &=\overline{U_{r}}\cap (X -(U\cup U_{s}))
  %\end{align*}
  %and 
  \begin{align*}
    \varnothing 
    ...
    &=\bigcap_{s<r<t}\overline{U_{r}-(U\cup U_{s})}\\
    &=\bigcap_{s<r<t}(\overline{U_{r}- U_{s}}\cap \overline{U_{r} - U})\\
    &=\bigcap_{s<r<t}\overline{U_{r}- U_{s}}\cap \bigcap_{s<r<t}\overline{U_{r} - U}\\
    &=Z_{s}\cap \bigcap_{s<r<t}\overline{U_{r} - U}\\
    &=Z_{s}\cap \bigcap_{s<r<t}\overline{U_{r}}\cap \overline{X- U}\\
    &=Z_{s}\cap \bigcap_{s<r<t}\overline{U_{r}}\cap (X- U)\\
    &=Z_{s}\cap (X- U)\cap \bigcap_{s<r<t}\overline{U_{r}}\\
    &=\varnothing. 
  \end{align*}
  This is a contradiction, 
  and it follows that \(\tilde{\phi}\) is an isomorphism. 

  \textcolor{black}{Since \(\tilde{\phi}\) is an} isomorphism, 
  it is enough to show that
  \[
  ...
  Let \(
    \sigma\in \Gamma(\textcolor{black}{Z_{s}\cup U_{s}};F)
  ...
  it follows from the compatibility of restrictions of a section} 
  that \(
    \mres[\tau]{Z_{s}}
    =
  ...
  \). 
  \textcolor{black}
  {B}y the definition of a section 
  ...
  \begin{equation}\label{eq:clm-glue}
    \textcolor{black}{U_{s}\cap W = U_{s}\cap \widetilde{W},
    \quad
  ...
  glue \(\textcolor{black}{\mres[\sigma]{U_{s}}}\) 
...
The author would like to thank Naofumi Honda, his supervisor for 
patient guidance. 
The autor would like to thank Yoko Umeta for her intererst 
and attending the seminars. 
The author also thanks Ryosuke Sakamoto, 
Kango Ito, and Yoshiaki Yamamura for fruitful discussions. 
In particular, Sakamoto gave him several ideas 
that is contained in the paper. 
The author would like to thank Yuichi Ike 
for helpful discussions. 
The author would like to thank Masato Tanabe 
for checking the draft. 
