...
However, we shall apply them to study some geometrical problems. 
In particular, we shall first prove a truncated version 
of the noncharacteristic deformation lemma, that is, 
if we weaken a hypothesis of the lemma and add 
a certain topological condition to the family of deformation, 
we can prove a finer result of it. 
Then, in turn, we can formulate 
the celebrated microlocal morse lemma, 
...
and by \(i\colon Z\hookrightarrow X\) an inclusion. 
...
A paracomact space is by definition a Hausdorff space. 
...
\subsection*{A Functorial Property under Proper Direct Images} 
\begin{Proposition+}
...
Because the original proof of the lemma cannot be applied 
to our truncatd version, we make an additional assumption of 
the family of deformation. 
Therefore, to make this assumption acceptable to the reader, 
we give several examples of families of deformation
before the proof. 

\begin{Proposition+}[Truncated Noncharacteristic Deformation Lemma]\label{prp:t-nonchar}
...
\begin{Example+}\label{eg:t-nonchar}
  ...
  \begin{enumerate}[(i)]
    ...
    Moreover, let us denote by \(Z\) the locally closed subset
    \[
      Z\coloneqq \left\{(x,y)\in X;\ (x+2)^{2}+(y-1)^{2}=1\right\}\setminus \left\{(-1,0)\right\}. 
    \]
    Then we denote by \(\kk_{Z}\) the sheaf on \(X\). 
    If \(j\leqq0\), we have \(
...
\begin{proof}
  We will use Proposition \ref{prp:t-nonchar}. 
  ...
  \end{enumerate}
  Then we can apply Proposition \ref{prp:t-nonchar}, and 
...
\begin{Theorem+}\label{thm:t-morse}
  ...
  \]
  are isomorphic for each \(j< k\), and 
  injective for \(j=k\).
\end{Theorem+}
...
teaching us all the important things about mathematics. 
...
for giving us some fundamental ideas of this paper. 
%===============================================
% 参考文献スペース
%===============================================
