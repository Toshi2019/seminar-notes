...
\section{Truncated Noncharacteristic Deformation}\label{sec:t-nonchar}
...
\begin{Proposition+}\label{prop:t-nonchar}
  Let \(X\) be a 
  locally compact (i.e., by definition, Hausdorff) space 
  countable at infinity (i.e., countable union 
  of compact subspaces). Let
  \(F\in\Dompb(\zz_X)\), and 
  ...
  \begin{enumerate}[(i)]
    ...
    For all pairs \((s,t)\) with \(s\leqq t\), 
    and all \(x\in Z_{s}\smi U_{t}\), we have\[
    ...
    For any \(s\in\rr\), \(Z_s\subset U_{t}\).
    %\item\footnote{
...
\begin{Remark+}
  Let us give an example that satisfies 
  the hypotheses 
  \eqref{item:t-nonchar1}--\eqref{item:t-nonchar3} 
  but not \eqref{item:t-nonchar4}. 
  Let us define a locally closed subset \(Z\) of \(\rr^2\) by\[
    Z\coloneqq\left\{
      (x,y)\in\rr^2
      \mathrel{;}
      (x+2)^{2}+(y-1)^{2}=1\right\}\smallsetminus\left\{(-2,0)\right\}.
  \]
  Consider a real valued function \(\varphi_{s}\) for 
  a real number \(s\): \begin{equation}
    \varphi_{s}(x)\coloneqq\begin{cases}
      0&\left|x\right|\geqq 2,\\
      \max(0,s(2-\left|x\right|))&\left|x\right|\leqq 2.
    \end{cases}
  \end{equation}
  We define \(\left(U_t\right)_{t\in\rr}\), 
  a family of open sets of \(\rr^2\) by
  \[
    U_t\coloneqq\left\{
      (x,y)\in\rr^2\mathrel{;}
      y<\varphi_{s}(x)
    \right\}.
  \]
  Then \(F\coloneqq \kk_{Z}\) and \(\left(U_t\right)_{t}\) 
  satisfy the hypotheses for \(k=0\). 
  Therefore, the natural morphism\[
    H^{0}(U_{1};\kk_Z)=\kk\to
    H^{0}(U_{0};\kk_Z)=0
  \]
  is not injective and the consequence of 
  Proposition \ref{prop:t-nonchar} does not hold.
\end{Remark+}
...
\begin{proof}[Proof of Proposition 5.1.]
  ...
  \begin{align}
      ...
      \lambda_{s}^{j}&\colon
      H^{j}(U_t;F)\simar \prolim[s<t] H^{j}(U_s;F).
  \end{align}
