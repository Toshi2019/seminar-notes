...
\begin{Proposition+}\label{prop:t-nonchar}
  ...
  \begin{enumerate}[(i)]
    ...
    \]
  \end{enumerate}
  Then we have for all \(t\in\rr\), the isomorphisms\[
...
\begin{proof}
  ...
  \begin{align}
      ...
      \indlim[t>s] H^{j}(U_t;F) &\simarr H^{j}(U_s;F),\\
      \tag*{\((b)^j_t\)}\label{eq:t-nonchar-b}
      \prolim[s<t] H^{j}(U_s;F) &\simrra H^{j}(U_t;F).
  \end{align}
  ...
  Consider the distinguished triangle\[
    \mres[(\RG_{X\smi U_{t}}(F))]{Z_{s}}\to
    \mres[(\RG_{X\smi U_{s}}(F))]{Z_{s}}\to
    \mres[(\RG_{U_{t}\smi U_{s}}(F))]{Z_{s}}\overset{+1}{\longrightarrow}.
  \] 
  By hypothesis \eqref{item:t-nonchar3}, it follows that
  \[
      \mres[(H^{j}_{X\smi U_{t}}(F))]{Z_{s}}=0,
      \quad
      \mres[(H^{j}_{X\smi U_{s}}(F))]{Z_{s}}=0
  \]
  for any \(j\leqq k\) and then \[
    \mres[(H^{j}_{U_{t}\smi U_{s}}(F))]{Z_{s}}=0 
    \quad
    \text{for any \(j\leqq k-1\).}
  \] 
  Thus for any \(j<k\) and \(t\geqq s\), we have
  \begin{align}
    0
    &=
    \Gamma(Z_{s};H^{j}_{U_{t}\smi U_{s}}(F))\\
    &=\indlim[U\supset Z_{s}]H^{j}(U\cap U_{t};H^{})
  \end{align}
\end{proof}
