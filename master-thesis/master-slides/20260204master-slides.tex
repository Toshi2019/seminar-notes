...
\begin{frame}
    \frametitle{研究の概要}
    \[\vcenter{\xymatrix@C=42pt@R=42pt{
        \txt{\(\muS[](F)\)\\
        \cite{KS82}}
        %\ar[dd]
        &
...
\begin{frame}
    ...
    \end{exampleblock}
    多様体\(M\)上の層（の複体）\(F\in\Dompb(\kk_{M})\)に対して，
    余接束の部分集合\(\muS[](F)\)を次で定める．
    \begin{DFN}[層の超局所台]
        \(p\notin \muS(F)\)であるとは，
...
\begin{frame}\frametitle{層に関する臨界点 --- モース理論との類似}
    ...
    \end{DFN}
    \begin{alertblock}{注意}
        超局所台は，すべての次数のコホモロジーが延長できるか否かを判定する概念．
    \end{alertblock}
    \vspace{60pt}
\end{frame}
...
\begin{frame}\frametitle{打切り超局所台 --- \(k\)次までの特異性}
    ...
    柏原--Fernandes--Schapira \cite{KFS03a}は，
    このような現象を扱うために
    超局所台を精密化した．
...
\begin{frame}
    ...
    \begin{itemize}
        \item \(M\): \(C^{\infty}\)多様体．
        \item \(F\in\Dompb(\kk_{M})\).
        \item \(\psi\colon M\to \rr\)と\(t\in\rr\)に対し，\(M_{t}\coloneqq \psi^{-1}((-\infty,t))\).
    \end{itemize}
    \begin{THM}[超局所モースの補題]
        \(\psi\colon M\to \rr\)を\(a\leqq\psi(x)< b\)で
        \(d\psi(x)\notin\muS[](F)\)となる\(C^1\)級関数で
        \(\supp(F)\)上固有なものとする．
        このとき制限射\[
            H^{j}(M_b;F)\longrightarrow H^{j}(M_a;F)
        \]は各\(j\in\zz\)について同型である．
    \end{THM}    
...
\begin{frame}
    \frametitle{主結果1}
    \begin{THM}[O., 打切り版超局所モースの補題]
        \textcolor{brown}{\(k\in\zz\)とする．}
        \(\psi\colon M\to \rr\)を\(a\leqq\psi(x)< b\)で
        \(d\psi(x)\notin\muS[\textcolor{brown}{k}](F)\)となる\(C^1\)級関数で
        \(\supp(F)\)上固有なものとする．
        このとき制限射\[
            H^{j}(M_b;F)\longrightarrow H^{j}(M_a;F)
...
\begin{frame}
  ...
  \begin{THM}[非特性変形補題]
    ...
    \begin{enumerate}[(i)]
      ...
      任意の実数\(s\leqq t\)と任意の点\(x\in Z_{s}- U_{t}\)に対し，
      \(
        (H^{j}_{X- U_{s}}(F))_{x}=0\quad (j\in\zz). 
      \)
    \end{enumerate}
    このとき，任意の\(t\in\rr\)に対し，次の射は同型である．\[
      H^{j}\left(\bigcup_{s\in\rr}U_{s};F\right)
      \to
      H^{j}\left(U_{t};F\right)\quad(j\in\zz).
    \]
  \end{THM}
\end{frame}
...
\begin{frame}
  ...
  \begin{THM}[O., 打切り版非特性変形補題]
    ...
    \begin{enumerate}[(i)]
      ...
      任意の点\(x\in Z_{s}- U_{\textcolor{brown}{s}}\)に対し，
      \textcolor{brown}{\(j\leqq k\)ならば}
      \(
        (H^{j}_{X- U_{s}}(F))_{x}=0. 
      \)
      \textcolor{brown}{\item \label{item:t-nonchar4} 
...
\begin{frame}
  \frametitle{仮定 (iv) をみたさない例}
    ...
    \(
      ...
      \right\}
    \)とすると
    \((U_{t})_{t}\)は\eqref{item:t-nonchar4}をみたさない．
      \begin{figure}[htb]
      \centering
...
\begin{frame}
  ...
  \end{equation*}
  仮定\eqref{item:t-nonchar3}から第1項が\(0\)であることがわかる．
\end{frame}

\begin{frame}
  \frametitle{証明の概要 (2/2)}
  したがって，次が成り立つ．
  \begin{align*}
    %\label{eq:a-iso}
...
\begin{frame}
    \frametitle{まとめ}
    \[\vcenter{\xymatrix@C=42pt@R=32pt{
        \txt{\(\muS[](F)\)\\
        \cite{KS82}}
        %\ar[dd]
        &
