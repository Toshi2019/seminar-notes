...
\begin{leftbar}
\begin{PRP}
    \(F\in\Dompb(X;p_X)\)とする．
\begin{enumerate}[(i)]
    \item \(p\)のある近傍で\(
        \mdiff[f]^{-1}(p_Y)\cap \mpb[f]^{-1}(\muS(F))
        \subset \left\{p\right\}
    \)であるとする．
    このとき，\(\muinv[f]F\)と\(\muexinv[f]F\)はともに\(
        \Dompb(Y;p_Y)
    \)に属し，標準射\(\omega_{Y/X}\tens[]\muinv[f]F\to\muexinv[f]F\)は同型である．
    さらに，\(p\)の任意の近傍\(W\)に対し，次がなりたつ．
    \begin{equation}\label{eq:6.1.26}
        \muS(\muinv[f]F)\subset \mdiff[f]^{-1}(W\cap \mpb[f]^{-1}(\muS(F)))\quad \text{on a neighborhood of \(p\).}
    \end{equation}\label{item:6.1.9-1}
    \item \(F\in\Dompb(X)\)が次の条件をみたすとする．\begin{enumerate}[(a)]
        \item \(f\)は\(F\)に関して非特性的である．\label{item:6.1.9-2a}
        \item \(\mdiff[f]^{-1}(p_Y)\subset \mpb[f]^{-1}(\muS(F))\subset \left\{p\right\}\).\label{item:6.1.9-2b}
    \end{enumerate}
    このとき\(\muinv[f]F\cong f^{-1}F\)かつ\(\muexinv[f]F\cong f^{!}F\)である．\label{item:6.1.9-2}
\end{enumerate}
\end{PRP}
\end{leftbar}
...
\begin{proof}
    \(p_X\in \conm[X]{X}\)のとき，
    \(f\)は\(F\)に関して非特性的であり，命題5.4.13から結論がしたがう．
    \begin{CMT}
        まず，\(p\in \conm[X]{X}\)の近傍\(\varOmega\)で\eqref{item:6.1.9-1}の仮定がなりたつとする．
        \(\varOmega_X\coloneqq \mpb[f](\varOmega)\)は\(p_X\)を含む．
        \(U_X\coloneqq \pi(\varOmega_X)\)で\(F\)の台を切って\(p_X\)で同型な射\(F'\to F\)をつくる．
        
        この準備のもとで\(f\)が\(F\)に関して非特性的であること，すなわち，
        \[
            \mpb[f]^{-1}(\muS(F))\cap\conm[Y]{X}\subset Y\dpb[X]\conm[X]{X}
        \]であることを示す．
        \(q\in \mpb[f]^{-1}(\muS(F))\cap\conm[Y]{X}\)とする．
        次の3つの場合を考える．
        \begin{enumerate}[(1)]
            \item \(q\in \pi^{-1}\Int[]U_X\),
            \item \(q\in Y\dpb[X]\cotb[X]-\pi^{-1}\Cl[]U_X\),
            \item \(q\in \pi^{-1}\Bd[]U_X\).
        \end{enumerate}
        
        (1)のとき，\(q\)は\(Y\tpb[X]\conm[X]{X}\)の点である．
        
        (2)のとき，\(q\)は\(\supp(F)\)に含まれない．
        
        (3)のとき，\(U'_X\Subset U_X\)となる\(\pi(p_X)\)の近傍を取り直して\(F'\)の台をさらに切ることで(2)の場合に帰着できる．
        ...
        \begin{quote}
            \begin{enumerate}[(i)]
                \item \(\muS(f^{-1}F)\subset \mdiff[f]\mpb[f]^{-1}\muS(F)\),
                \item \(f^{-1}F\tens[]\omega_{Y/X}\simar f^{!}F\).
            \end{enumerate}
        \end{quote}
        から，\(p_X\)上の同型\(F'\to F\)に対し，
        \(f^{-1}F'\to f^{-1}F\)は\(p_Y\)上の同型である．
        したがって，\[
            \Hom_{\Dompb(Y;p_Y)}(f^{-1}F,\blk)
            \to
            \Hom_{\Dompb(Y;p_Y)}(f^{-1}F',\blk)
        \]は全単射である．これから
        \[
            \Hom_{\Dompb(Y;p_Y)}(f^{-1}F,\blk)
            \simarr
            \prolim[F'\to F]\Hom_{\Dompb(Y;p_Y)}(f^{-1}F',\blk)
        \]
        がなりたつので，
        \(f^{-1}F\)は\(\muinv[f]F\)を表現する対象である．
        よって，\(\muinv[f]F\cong f^{-1}F\in \Dompb(Y;p_Y)\)である．
    \end{CMT}

    \(p_X\in\cotn[X]\)であるとする．
    まず\(\muinv[f]\)に関する結論を示す．
    \(F\)を\(\Dompb(X;p_X)\)の対象とする．
    \(x_0=\pi(p_X)\), \(y_0=\pi(p_Y)\), \(V=\Ker(\conm[{x_0}]{X}\to\conm[{y_0}]{Y})\)とおく．
    固有閉凸錐\(K\)と開凸錐\(U\)で，\[
        p_X\in U\subset K,\quad
        K\cap V\subset\left\{0\right\},\quad
        \mpb[f]\mdiff[f]^{-1}(p_Y)\subset\muS(F)\cap K \subset\left\{p_X\right\}
    \]をみたすものをとる．

    refined microlocal cut-off lemma から，
    射\(u\colon F'\to F\)で，
    \(p_X\)において同型であり，
    \(F'\)が条件\eqref{item:6.1.9-2a}と\eqref{item:6.1.9-2b}を
    みたすものがある．

    さらに，
    %refined microlocal cut-off lemma から，
    \(F''\to F'\)が\(p_X\)での同型かつ\(F'\)と\(F''\)が\eqref{item:6.1.9-2}の条件をみたすならば，
    この射を完全三角\(F''\to F'\to F_\circ\to +1\)にうめこむことで，
    \(f\)が\(F_\circ\)に関して非特性的であることと\(\mdiff[f]^{-1}(p_Y)\cap \mpb[f]^1(\muS(F_\circ))=\varnothing\)であることがわかる．
    よって命題5.4.13から\(p_Y\notin \muS(f^{-1}F_\circ)\)である．
    つまり\(f^{-1}F''\to f^{-1}F'\)は\(\Dompb(Y;p_Y)\)において同型である．
    \(\muinv[f]F\)の定義から，
    ind-lim は射\(F'\to F\)で\(F'\)が\eqref{item:6.1.9-2}の条件をみたすものからなる圏をわたる．
    よって，どの\(f^{-1}F'\)も\(\Dompb(Y;p_Y)\)においては同型であり，
    これから\(\muinv[f]\)に関する\eqref{item:6.1.9-1}と\eqref{item:6.1.9-2}の主張が示される．
    \(\muexinv[f]\)に関する主張は，
    dual microlocal cut-off lemma から同様に証明でき，
    同型\(\omega_{Y/X}\tens[]\muinv[f]F\simar \muexinv[f]F\)は
    \eqref{item:6.1.9-2}と命題5.4.13からしたがう．
\end{proof}
