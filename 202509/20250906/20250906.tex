...
\subparagraph{設定}
\begin{itemize}
    \item \(\pi\colon E\to M\): ベクトル束
    \item \(\nabla^{E}\): \(E\)上の接続
    \item \(c\colon [a,b]\to M\): \(C^{\infty}\)級曲線
\end{itemize}
このとき
未知関数\(\gamma\in \Gamma(c^{\ast}E)\)に関する
初期値問題
\begin{equation}
    \begin{dcases}
        \nabla^{c^{\ast}E}_{\frac{d}{dt}}\gamma=0\in\Gamma(c^{\ast}E),\\
        \gamma(a)=\xi\in E_{c(a)}    
    \end{dcases}
\end{equation}
を考える．
