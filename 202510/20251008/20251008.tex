...
\(A\times\rrp\)の点列\[
    \left(x_n',x_n'';\xi'_n,\xi''_n,c_n\right)_n
\]で\[\begin{dcases}
...
\begin{proof}
    (i) 
    まず，\(\wncone[{\conm[M]{X}}](A)\)が2重錐状 (biconic) であることを示す．
    \(
        (0,x_0'',\xi_0',0;x_0',0,0,\xi_0'')
        \in \norb[{\conm[M]{X}}]{\cotb[X]}
    \)であるとする．
    ...
    \]
    とおくと，\(\left(
        \tilde{x}_n',\tilde{x}_n'';\tilde{\xi}'_n,\tilde{\xi}''_n,
        \tilde{c}_n
    ...
    とおくと，\(A\)が錐状であることから\(\left(
        \check{x}_n',\check{x}_n'';\check{\xi}'_n,\check{\xi}''_n,
        \check{c}_n
    \right)_n\)は\(A\times \rrp\)の点列であり，
    \begin{align*}
...
\begin{PRP}[{\cite[Prop. 4.1.2]{KS90}}]\label{prp4.1.2}
    (ii) 
    \((x)=(x',x'')\)を\(X\)の局所座標系とし，
    \(M=\left\{x;x'=0\right\}\)とする．
    \(S\)を\(X\)の部分集合とする．
    \(x_{\circ}=(0,x''_{\circ})\in M\), 
    \((x''_{\circ};v_{\circ})\in \norb[M]{X}\)とする．
    このとき，
    \((x_{\circ};v_{\circ})\in\wncone[M](S)\)となるのは，
    \(S\times \rrp\)の点列で，\(x_n=(x'_n;x''_n)\)とおくとき，\(x_n\underset{n}{\to}x_{\circ}\), \(c_nx'_n\underset{n}{\to}v_\circ\)をみたすものが存在するときである．
\end{PRP}
\end{leftbar}
%\section{aa}
%===============================================
