...
次の記号を用いる．
\begin{description}
    \item[環] \(k\)は可換環，特に\(\zz\)か体を表す．
    \item[テンソル積] \(\tens[]\)で\(k\)上のテンソル積を表す．
    \item[\(k\)代数] \(k\)代数とは，\(k\)加群\(A\)に\(k\)線形かつ結合的な積\(A\tens[]A\to A\)を合わせたもので両側単位元\(1\in A\)をもつものをいう．
    \item[\(k\)圏] \(k\)圏\(\cat[A]\)とは，「多対象版\(k\)代数」のことである．
    すなわち次のデータの組のことである．\begin{itemize}
        \item 対象のクラス\(\Ob(\cat[A])\)
        \item \(\cat[A]\)の任意の対象\(X,Y\)に対して定まる
        \(k\)加群\(\Hom_{\cat[A]}(X,Y)\)
        \item \(k\)線形かつ結合的な合成写像\[
            \Hom_{\cat[A]}(Y,Z)\tens[]\Hom_{\cat[A]}(X,Y)
            \to\Hom_{\cat[A]}(X,Z),
            \ (f,g)\mapsto fg
        \]で単位射\(1_X\in \Hom_{\cat[A]}(X,X)\)をもつもの
    \end{itemize}
    例えば，\(k\)代数は対象がひとつの\(k\)圏とみなせる．
    \(k\)代数\(A\)上の右加群の圏\(\Mod(A^{\op})\)は多数の対象をもつ
    \(k\)圏の例である．
    この圏は\(k\)線形圏，
    すなわち\(k\)圏ですべての有限直和を含むものの例でもある．
\end{description}

\begin{DFN}
    \begin{enumerate}[(i)]
        \item \textbf{次数（つき）\(k\)加群} (graded \(k\)-module) は
        \(k\)加群\(V\)に，整数によって添字づけられた分解\[
            V=\bigoplus_{p\in\zz}V^{p}
        \]をあわせたものをいう．
        \item \(V\)に対し\(V[1]\)を\(
            V[1]^{p}=V^{p+1}
        \) \((p\in \zz)\)とおくことで定める．
        \item \(n\)次の次数\(k\)加群の射\(f\colon V\to V'\)とは
        \(k\)線形射ですべての\(p\in\zz\)に対し\(f(V^{p})\subset V^{p+n}\)をみたすものをいう．
        \item \(V\tens[]W\)を第\(n\)成分を\[
            (V\tens[]W)^{n}=\bigoplus_{p+q=n}V^p\tens[]W^q
        \]とおくことで定める．
    \end{enumerate}
...
\begin{thebibliography}{20} 
  ...
  \bibitem[GP74]{GP74} Victor Guillemin, Alan Pollack, 
    ...
    Prentice-Hall, 1974.
  \bibitem[Kel06]{Kel06} Keller, \textit{On Differential Graded Categories}, 2006.
