...
\section*{Preface}
In these notes, 
We study microlocal Morse theory, which is developed by
Guillermou, Kashiwara, and Schapira in \cite{GKS12}, 
but replace microsupports with
truncatd Microsupports introduced by 
...
\begin{Proposition+}
  ...
  \]
  are isomorphic for each \(j\leqq k\).
\end{Proposition+}
...
\begin{proof}
  First, we have\[
    H^{j}(M_{t};F)
    \cong
    H^{j}\RG({]-\infty,t[}\mathrel{;}\Rder{\psi}_{\ast}F)
  \]
  for \(t\in I\).
  Since we assume that \(d\psi(x)\notin \muS[k](F)\) 
  for \(a\leqq \psi(x)<b\), and by the formula \(\muS[k](\Rder{\psi}_{\ast}F)
    \subset \mpb[\psi]\mdiff[\psi]^{-1}\muS[k](F)
  \), we have\[
    \left(H^{j}\RG_{[a,b[}\Rder{\psi}_{\ast}F\right)_{\psi(x)}=0
  \]for each \(j\leqq k\). 
  Therefore, letting \(d\in\zz\) be the minimum number 
  that \[H^{d}\RG_{]-\infty,a[}(\Rder{\psi}_{\ast}F)\ne0,\] 
  by the long exact sequence
  \begin{align*}
    0&\to 
    H^{d}\RG_{[a,b[}(\Rder{\psi}_{\ast}F)
    \to
    H^{d}\RG_{]-\infty,b[}(\Rder{\psi}_{\ast}F)
    \to
    H^{d}\RG_{]-\infty,a[}(\Rder{\psi}_{\ast}F)\\
    &\to 
    H^{d+1}\RG_{[a,b[}(\Rder{\psi}_{\ast}F)
    \to
    H^{d+1}\RG_{]-\infty,b[}(\Rder{\psi}_{\ast}F)
    \to
    H^{d+1}\RG_{]-\infty,a[}(\Rder{\psi}_{\ast}F)\\
    &\to\cdots\\
    &\to 
    H^{k}\RG_{[a,b[}(\Rder{\psi}_{\ast}F)
    \to
    H^{k}\RG_{]-\infty,b[}(\Rder{\psi}_{\ast}F)
    \to
    H^{k}\RG_{]-\infty,a[}(\Rder{\psi}_{\ast}F)\\
    &\to
    H^{k+1}\RG_{[a,b[}(\Rder{\psi}_{\ast}F)\to\cdots,
  \end{align*} 
  we have isomorphisms
  \begin{equation*}%\label{eq:t-morse-lemma}
    \left(H^{j}\RG_{]-\infty,b[}(\Rder{\psi}_{\ast}F)\right)_{\psi(x)}
    \simarr
    \left(H^{j}\RG_{]-\infty,a[}(\Rder{\psi}_{\ast}F)\right)_{\psi(x)}
  \end{equation*}
  for any \(j\leqq k\) and \(a\leqq \psi(x) < b\), 
  and then, 
  \begin{equation}\label{eq:t-morse-lemma}
    H^{j}\RG_{]-\infty,b[}(\Rder{\psi}_{\ast}F)
    \simarr
    H^{j}\RG_{]-\infty,a[}(\Rder{\psi}_{\ast}F)
  \end{equation}
  for all \(j\leqq k\). Then we can conclude that
  \[
    H^{j}(M_b\mathrel{;}F)
    \simarr
    H^{j}(M_a\mathrel{;}F)
  \]
  by applying to the equality \eqref{eq:t-morse-lemma} 
  the functor \(
    \Gamma({]-\infty,b[}\mathrel{;}\blk)
  \) and calculating as follows:
  \begin{align*}
    \Gamma({]-\infty,b[}\mathrel{;}H^{j}\RG_{]-\infty,b[}(\Rder{\psi}_{\ast}F))
    &\cong
    \Gamma_{]-\infty,b[}({]-\infty,b[}\mathrel{;}\Rder^{j}{\psi}_{\ast}F)\\
    &\cong
    \Gamma({]-\infty,b[}\mathrel{;}\Rder^{j}{\psi}_{\ast}F)\\
    &\cong
    H^{j}({\psi}^{-1}\left({]-\infty,b[}\right)\mathrel{;}F)\\
    &\cong
    H^{j}(M_{b}\mathrel{;}F)
  \end{align*}
  and
  \begin{align*}
    \Gamma({]-\infty,b[}\mathrel{;}H^{j}\RG_{]-\infty,a[}(\Rder{\psi}_{\ast}F))
    &\cong
    \Gamma_{]-\infty,a[}({]-\infty,b[}\mathrel{;}\Rder^{j}{\psi}_{\ast}F)\\
    &\cong
    \Gamma({]-\infty,a[}\mathrel{;}\Rder^{j}{\psi}_{\ast}F)\\
    &\cong
    H^{j}({\psi}^{-1}\left({]-\infty,a[}\right)\mathrel{;}F)\\
    &\cong
    H^{j}(M_{a}\mathrel{;}F).
  \end{align*}
\end{proof}

\section{Truncated Morse Inequalities}
The main theorem of \cite{ST92} is the following: 
we assume
\begin{align}\label{eq:ST92-H1}
  \text{\(\psi^{-1}(]-\infty,t[)\cap \supp(F)\) is compact for any \(t\in\rr\)}, \tag{H.1}\\
  \text{there is a finite family \(\left\{K_{j}\right\}_{1\leqq j\leqq N}\)of disjoint compact}\notag\\
  \text{subsets in \(X\) and real numbers \(\left\{s_{j}\right\}_{1\leqq j\leqq N}\) such that}\notag\\
  \pi(\Lambda_{\psi}\cap \Lambda)=\bigcup_{j}K_{j},
  \quad
  \mres[\psi]{K_{j}}=s_j,\tag{H.2}\label{eq:ST92-H2}\\
  \text{\(V_{j}\coloneqq
  \RG(K_{j};\RG_{\left\{\psi\geqq s_{j}\right\}}(F))\) belongs to \(\Dompb(\Modf(\kk))\).}\tag{H.3}\label{eq:ST92-H3}
\end{align}

Set 
\begin{equation}
  n_{l}\coloneqq \sum_{j}\dim H^{l}(V_j).  \label{eq:ST92-12}
\end{equation}
Then 
\begin{Theorem+}[Generalized Morse Inequalities {\cite[Theorem~1.1]{ST92}}]
  Assume \eqref{eq:ST92-H1}, \eqref{eq:ST92-H2}, and \eqref{eq:ST92-H3}. 
  Then
  \begin{enumerate}[(i)]
    \item \(\RG(X;F)\in\Dompb(\Modf(\kk))\),
    \item setting \(b_{l}(X,F)= b_{l}(\RG(X;F))\), we have the inequalities\[
      b_{l}^{\ast}(X,F)\leqq n_{l}^{\ast}
    \] where \(n_{l}\) is given by \eqref{eq:ST92-12}.
  \end{enumerate}
\end{Theorem+}
