...
% documentclass
% ------------------------
\documentclass[10pt, a4paper, leqno, dvipdfmx]{jsarticle}
...
\begin{DFN}[層]\label{def:sheaf}
    \(X\)を位相空間とする．
    \(\mathscr{F}\colon\Op_{X}^{\op}\to\cat\)を
    \(X\)上のアーベル圏\(\cat\)に値をとる前層とする．
...
%        \item \(\mathscr{F}(\varnothing)=0\).
        ...
        \[\vcenter{\xymatrix
            ...
            &
            \mathscr{F}(U)
            \ar[rr]^-{\prod_i\rho_{U_iU}}
            &&
            \dip \prod_{i\in I}\mathscr{F}(U_i)
        }}\]が完全である．
        すなわち，\(s\in\mathscr{F}(U)\)が各\(i\in I\)に対して\(U_i\)上\(s\rvert_{U_{i}}=0\)ならば，
        \(s=0\)である．\label{sheaf-cond1}
        \item 列
        \[\vcenter{\xymatrix
            @C=16pt@R=28pt{
            \mathscr{F}(U)
            ...
            &&
            \dip \prod_{i\in I}\mathscr{F}(U_i)
            \ar[rr]^-{\prod_{i,j}\varphi_{ij}}
            &&
            \dip \prod_{i,j\in I}\mathscr{F}(U_i\cap U_j)
        }}\]が完全である．
        ただし，\(\varphi_{ij}=\rho_{U_{i}\cap U_{j},U_i}-\rho_{U_{i}\cap U_{j},U_j}\). 
        すなわち，\(\left(s_i\right)_i\in\prod_{i}\mathscr{F}(U_i)\)が
...
\begin{PRP}
複素直線\(\cc\)上の複素線型空間の前層\(\scF\)を次で定める．
\[
    \scF(U)=
    \begin{cases}
        \cc &\text{\(U=\cc\)のとき，}\\
    ...
    \end{cases}
\]
\(\scF\)は層の条件S\ref{sheaf-cond2}をみたすがS\ref{sheaf-cond1}は
みたさない．したがって，\(\scF\)は層にならない．
\end{PRP}
\begin{proof}
    \subparagraph*{条件S\ref{sheaf-cond2}をみたすこと：}
    \(U\subsetneqq\cc\)を\(\cc\)の開集合とし，
    \(\left(U_i\right)_{i\in I}\)を
    \(U\)の開被覆とする．
    \(\left(s_i\right)_i\in\prod_{i}^{}\scF(U_i)\)を
    各\(U_i\cap U_j\)で\(
        s_i\rvert_{U_i\cap U_j}=s_j\rvert_{U_i\cap U_j}
    \)をみたす切断の族とする．
    どの\(i\)に対しても\(U_i\subset U\subsetneqq\cc\)であることから
    \(\scF(U_i)=0\)である．すなわち\(s_i=0\)である．
    したがって，\(0\in\scF(U)\)は各\(i\in I\)に対し\(
        0\rvert_{U_i}=0=s_i
    \)をみたす．
    ...
    各\(U_i\cap U_j\)で\(
        s_i\rvert_{U_i\cap U_j}=s_j\rvert_{U_i\cap U_j}
    \)をみたす切断の族とする．このとき，
    \(U_i\neq \cc\)となる\(i\)に対して\(s_i=0\)となるので，
    \(U_i\)が\(\cc\)の真部分集合からなる場合は\(0\in\scF(\cc)\)が\(
        0\rvert_{U_i}=0=s_i
    \)をみたす．
    \(U_i=\cc\)となる\(i\)があれば，そのような\(i\)たちに対して，
    仮定\(
        s_i\rvert_{U_i\cap U_j}=s_j\rvert_{U_i\cap U_j}
    \)から切断はすべて\(s_i\)としてよい．よって
    \(s_i\in\scF(\cc)\)とすると，
    \(U_j=\cc\)上では\(s_i\rvert_{U_j}=s_i=s_j\)が成り立ち，
    \(U_j\subsetneqq\cc\)上では\(s_i\rvert_{U_j}=0=s_j\)となる．

    \subparagraph*{条件S\ref{sheaf-cond1}をみたさないこと：}
    \(U=\cc\)とする．開集合\(U_0\), \(U_1\)を
    \[
        U_0=\left\{z\in\cc;\Re{z}>-1\right\}, \quad
        U_1=\left\{z\in\cc;\Re{z}< 1\right\}
    \]とおくと，
    \(\cc=U_0\cup U_1\)である．
    \(s_0=0\in\scF(U_0)\), \(s_1=0\in\scF(U_1)\)とすると
    \[
        s_0\rvert_{U_0\cap U_1}=0=s_1\rvert_{U_0\cap U_1}
    \]が成り立つ．
    ところが，\(s=1\in\cc=\scF(U_0\cup U_1)\)とすると，
    \(s\)は\(0\)ではないが\[
        s\rvert_{U_0}=0=s_0, \quad s\rvert_{U_1}=0=s_1
    \]をみたす．
\end{proof}
