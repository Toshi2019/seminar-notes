...
\section{関手\(\muhom\) {\cite{Sch88}}}
\begin{itemize}
    \item \(X\): \(C^\infty\)多様体
    \item \(\pi\colon \cotb[X]\to X\): 余接束
    \item \(\Dompb(X)\): \(X\)上のアーベル群の層の複体でコホモロジーが有界なものからなる導来圏
\end{itemize}
...
\begin{itemize}
    ...
    \item \(j\ne m\)
\end{itemize}
に対し\(H^{j}(F)=0\)が成り立つことをいう．
\(F\)が次数\(0\)に集中しているとき，\(F\)と\(H^0(F)\)を同一視する．

\(M\)が\(X\)の部分多様体のとき，\(\conm[M]{X}\)で余法束を表す．

\paragraph{向き付け層}
\(\ori_X\)で\(X\)上の向き付け層を表す．
\(f\colon Y\to X\)が連続写像のとき，
\(\ori_{Y/X}=\ori_Y\tens[]f^{-1}\ori_X\)とおく．

\paragraph{ \(\muhom\) }
\(\micro{M}\colon\Dompb(X)\to\Dompb(\conm[M]{X})\)を超局所化関手とする．
\(X\times X\)から\(X\)への第\(i\)射影を\(q_i\)で表す．
\(X\times X\)の対角集合を\(\diag[]\)で表す．
このとき\(\Dompb(X)\)の対象\(F\)と\(G\)に対し
\begin{equation}\label{eq:def-muhom}
    \muhom(G,F)\coloneqq
    \micro{\diag[]}\RHOM(q_{2}^{-1}G,q_{1}^{!}F)    
\end{equation}
とおく．\(\muhom(G,F)\)は関手\[
    \muhom\colon 
    \Dompb(X)^{\op}\times \Dompb(X)
    \to 
    \Dompb(\conm[{\diag[]}]{X^2})
    \simar
    \Dompb(\cotb[X])
\]
を定める．
次が成り立つ．ただし，\(j\colon \conm[M]{X}\hookrightarrow \cotb[X]\)で閉うめこみを表す．
\begin{align}
    \muhom(\zz_M,\blk)&\cong j_{\ast}\micro{M}(\blk),\label{eq:muhom-vs-micro}\\
    \Rder{\pi_{\ast}}\muhom(\blk,\blk)&\cong\RHOM(\blk,\blk),\label{eq:muhom-to-hom}\\
    \supp(\muhom(G,F))&\subset\muS(G)\cap\muS(F).\label{eq:supp-muhom}
\end{align}

\begin{THM}
    \(U\)を\(\cotb[X]\)の開集合とし，
    \(h\)を\(U\)上の\(C^2\)級関数とする．
    \(\muS(F)\subset\left\{h\geqq0\right\}\)かつ
    \(\muS(G)\subset\left\{h\leqq0\right\}\)であるとし，
    \(u\)を\(H^{j}(\muhom(G,F))\)の\(U\)上の切断とする．
    このとき，\(\supp(u)\)は
    \(H_h\)の positive half-integral curves の和である．
\end{THM}
...
\begin{DFN}
    \[
        \mfun[A\mid X]=\muhom(\zz_A,\cO_X)\tens[]\ori_{M/X}[n]
    \]
    とおく．
    とくに\(
        \mfun[M\mid X]
        =\micro{M}(\cO_X)\tens[]\ori_{M/X}[n]
    \)は佐藤の超局所関数の (\(n\)次に集中した) 層である．
    これをたんに\({\mfun[M]}\)で表す．
\end{DFN}
\subparagraph{コメントと計算}
定義における\(\ori_{M/X}\)は\(\pi^{-1}\ori_{M/X}\)の
略記である（\cite[定義11.5.1]{KS90}の後の注意を参照）．
また，\eqref{eq:muhom-vs-micro}に\(j^{-1}\)を適用した
\[
    j^{-1}\muhom(\zz_M,\blk)\cong j^{-1}j_{\ast}\micro{M}\cong\micro{M}
\]
より，
\begin{align*}
    \mfun[M\mid X]
    &=\muhom(\zz_M,\cO_X)\tens[]\pi^{-1}\ori_{M/X}[n]\\
    &=j_{\ast}\micro{M}(\cO_X)\tens[]\pi^{-1}\ori_{M/X}[n]
\end{align*}
について
\[
    \mres[{\mfun[M\mid X]}]{\conm[M]{X}}
    \cong
    \micro{M}(\cO_X)\tens[]\pi^{-1}\ori_{M/X}[n]
    \cong
    \mfun[M]
\]
となる．
\eqref{eq:supp-muhom}から，\(\mfun[M]\)は\(\conm[M]{X}\)以外に
台をもたないので，\(j_{\ast}\mfun[M]\)と\(\mfun[M]\)を同一視してよい．
何が言いたいかというと，
\begin{align*}
    j_{\ast}\mfun[M]
    &\cong j_{!}j^{-1}\mfun[M\mid X]\\
    &\cong \left(\mfun[M\mid X]\right)_{\conm[M]{X}}
\end{align*}
なので，
\[\begin{cases}
    \mres[{j_{\ast}\mfun[M]}]{\conm[M]{X}}=\mfun[M],\\
    \mres[{j_{\ast}\mfun[M]}]{\cotb[X]-\conm[M]{X}}=0
\end{cases}\]
が成り立つのだということである．

さて，\eqref{eq:muhom-to-hom}から，
\begin{align*}
    \Rder{\pi_{\ast}}\mfun[A\mid X]
    &\cong
    \RHOM(\zz_A,\cO_X)\tens[]\ori_{M/X}[n]\\
    &\cong
    \RG_{A}(\cO_X)\tens[]\ori_{M/X}[n]\\
    &\cong 
    \Gamma_{A}(\Gamma_{M}(\cO_X)\tens[]\ori_{M/X}[n])\\
    &\cong 
    \Gamma_{A}(\hfun[M])
\end{align*}
が成り立つ．
ここで，\(
    \hfun[M]\coloneqq \RG_{M}(\cO_X[n])\tens[]\ori_{M/X}
\)は佐藤超関数の（\(0\)次集中の）層である．
\(A\)が閉半空間で境界が実解析的なものである場合の層
\(\mfun[A\mid X]\)は，片岡が別の方法で導入している．

\(\cO_X\)の脆弱次元は\(n\)なので，
複体\(\mfun[A\mid X]\)は次数\(\leqq0\)に集中している．

\eqref{eq:supp-muhom}から，\begin{equation}\label{eq:supp-mild-mfun}
    \supp(\mfun[A\mid X])\subset\muS(\zz_A)
\end{equation}
がなりたることに注意．
\(A\)が\(M\)の閉集合であるとし，\(\varOmega=M-A\)とおく．
完全系列\[0\to\zz_{\varOmega}\to\zz_{M}\to \zz_{A}\to0\]から，
次の完全三角が得られる．
\begin{equation}\label{eq:dt-mild-mfun}
    \mfun[A\mid X]\to \mfun[M]\to \mfun[\varOmega\mid X]\to+1.
\end{equation}
\(\conm[M]{X}\)の外では\(\mfun[M]=0\)なので，
\(\cotb[X]-\conm[M]{X}\)上で\(
    \mfun[A\mid X]\cong \mfun[\varOmega\mid X][-1]
\)となる．
したがって，
\(\mfun[\varOmega\mid X]\)は一般に\(0\)次集中とは限らない．
他方，\(H^{0}(\mfun[\varOmega\mid X])\)の台は\(\conm[M]{X}\)に含まれる．
...
\(\varOmega\)が\(M\)の開集合の場合を考える．
うめこみ\(M\hookrightarrow X\)を\(j\)で表す．
自然な射\begin{equation}
    \alpha\colon 
    \pi_{X}^{-1}\Gamma_{\varOmega}(\hfun[M])
    \to 
    H^{0}(\mfun[\varOmega\mid X])
\end{equation}
と
\begin{equation}
    \beta\colon 
