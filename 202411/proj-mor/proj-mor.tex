...
\documentclass[uplatex,dvipdfmx,b5paper,9pt,draft]{jsarticle} 
...
射影的射\(f\colon X\to Y\)は，代数閉体\(k\)上の代数多様体が
射影代数多様体と同型であることの一般化．

\section{定義}

\(V\)を\(k\)上の代数多様体とする．
\(\Proj{k[x_0,\dots,x_n]}=\pp_{k}^{n}\)，
射影\(n\)空間は\(\pp^n\)で表すことにする．

\section{本論}

\cite[Prop. 2.6]{Ha77}を用いると，\(t(\pp^n)=\pp_{k}^{n}\)となる．

\(t(V)=X\)とすると\(X\)は\(k\)上のスキームだから，
\(X\to\Spec{k}\)が存在する．
これが射影的であることを示す．
最後に\(t(i)\)が閉埋込であることを示す．
...
\(t(i)\colon Y\hookrightarrow\pp_{k}^{n}\)が閉埋め込みかを調べれば良い．
\[
    W=\left\{[a_0:\dots:a_n];
    f_0(a_0,\dots,a_n)=\dots=f_l(a_0,\dots,a_n)=0, 
    \text{\(f_i\)は斉次多項式} 0\leqq i\leqq l
    \right\}
\]
であるので，\(Y=\Proj{k[x_0,\dots,x_n]}/(f_0,\dots,f_l)\)となる．
したがって，\(Y\)は\(\pp_{k}^n\)の閉部分スキームであり，
\(t(i)\)は閉埋め込みである．
よって，代数閉体\(k\)上の代数多様体が\(\pp^n\)に埋め込めるとき，
これに対応する\(k\)上のスキーム\(X\)に付随する\(X\to\Spec{k}\)は射影的である．
%===============================================
% 参考文献スペース
%===============================================
\begin{thebibliography}{20} 
    %\bibitem[GP74]{GP74} Victor Guillemin, Alan Pollack, \textit{Differential Topology}, Prentice-Hall, 1974.
    \bibitem[Ha77]{Ha77} Robin Hartshorne, 
