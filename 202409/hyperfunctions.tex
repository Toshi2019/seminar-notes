...
\title{超関数について}
%\author{}
\date{\today\footnote{2024/9/5かきはじめ}}
...
\cite[Section.4]{Sch04}の超関数に関する記述を抜粋して述べる．
...
\(M\)を\(n\)次元実解析多様体とし，その上の層\(\cB_{M}\)を
\begin{equation}\label{eq:B-def1}
    \cB_{M}\coloneqq H^{n}_{M}(\cO_X)\ttens[]\ori_{M/X}    
\end{equation}
で定める．ただし，\(X\)は\(M\)の複素化\footnote{
    \(M\)がパラコンパクトならば，\(X\)は局所的には一意である．
}である．

\({\cc_{X}}_{M}\)で，\(\cc_{M}\)を\(X-M\)に0で拡張したものを表す．
ポアンカレ双対性により，\(M\)上で
\[
    \RHOM_{\cc_X}({\cc_{X}}_{M},\cc_{X})\cong\ori_{M/X}[-n].
\]
が成り立つ．
ここで，左辺の\(\RHOM_{\cc_X}({\cc_{X}}_{M},\cc_{X})\)は
\(M\)への制限\(
    i^{-1}\RHOM_{\cc_X}({\cc_{X}}_{M},\cc_{X})
\)と同一視している．

\begin{proof}
    \(i\colon M\hookrightarrow X\)を閉埋め込みとすると
    \begin{align*}
        i^{-1}\RHOM_{\cc_X}({\cc_{X}}_{M},\cc_{X})
        &\cong i^{-1}\RHOM_{\cc_X}(\Rder{i}_{!}\cc_{M},\cc_{X})\\
        &\cong i^{-1}\Rder{i}_{\ast}\RHOM_{\cc_M}(\cc_{M},i^{!}\cc_{X})\\    
        &\cong \RHOM_{\cc_M}(\cc_{M},\omega_{M/X})\\
        &\cong\omega_{M/X}\\
        &\cong\ori_{M/X}[-n]
    \end{align*}
    となる．
\end{proof}
\(X\)は向きづけ可能なので，\(\ori_X\cong\cc_X\)が成り立つ．
よって\eqref{eq:rel-ori1}で\(\ori_{M/X}\cong\ori_{M}\)と
なることから
\[    
    \rmD'_{X}({\cc_{X}}_{M})
    \coloneqq
    \RHOM_{\cc_X}({\cc_{X}}_{M},\cc_{X})
\]とおくと，
\[
    \rmD'_{X}({\cc_{X}}_{M})
    \cong\ori_{M}[-n]
...
ここでも\(X\)上の\(
    \RHOM_{\cc_X}(\rmD_{X}'{\cc_X}_{M},\cO_X)
\)を\(M\)への制限\[
    \mres[\RHOM_{\cc_X}(\rmD_{X}'{\cc_X}_{M},\cO_X)]{M}
\]と同一視している．
\eqref{eq:B-def1}と\eqref{eq:B-def2}が同値な定義であることは，
\[
    H^{n}_{M}(\cO_X)\tens[]\ori_{M}
    \cong
    \RG_{M}(\cO_{X})[n]\tens[]\ori_{M}
\]
であることから，次を示すことに帰着される．
\begin{equation}\label{eq:Bdefs}
    \RG_{M}(\cO_X)\ttens[]\ori_{M}[n]
    \cong 
    \RHOM_{\cc_X}(\rmD'_{X}{\cc_X}_M,\cO_X).
\end{equation}
\begin{proof}
