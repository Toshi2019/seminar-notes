...
%    \caption{色付きの表}
    ...
    \hline
        ...
        \hline
        165&問1&\(\textcolor{red}{\bm{u}}, \textcolor{red}{\bm{v}}\)&\(\textcolor{red}{\bm{v}}, \textcolor{red}{\bm{w}}\)&もとの問（p.5）の記号に合わせる．\\    
    \hline
\end{tabular}
\end{table}
...
\setcounter{PBM}{4}
\begin{PBM}
    \(\bm{l}(t)=\left(e^t\cos{t},e^t\sin{t}\right)\), 
    \(\bm{V}(x,y)=(x,y)\)とするとき，線積分\(
        \displaystyle
        \lim_{T\to\infty}\int_{-T}^{0}\bm{V}\cdot{d}\bm{l}
    \)を計算せよ．
\end{PBM}
\begin{proof}[解答]
    \begin{align*}
        \lim_{T\to\infty}\int_{-T}^{0}\bm{V}\cdot{d}\bm{l}
        &=\lim_{T\to\infty}\int_{-T}^{0}\left(
            e^t\cos{t},e^t\sin{t}
        \right)\cdot\left(
            e^t\cos{t}-e^t\sin{t},e^t\sin{t}+e^t\cos{t}
        \right)dt\\
        &=\lim_{T\to\infty}\int_{-T}^{0}\left(
            e^{2t}\cos^2{t}-e^{2t}\cos{t}\sin{t}
            +e^{2t}\sin^2{t}+e^{2t}\sin{t}\cos{t}
        \right)dt\\
        &=\lim_{T\to\infty}\int_{-T}^{0} e^{2t}dt\\
        &=\lim_{T\to\infty}\frac{1}{2}\left(1-e^{-2T}\right)
        =\frac{1}{2}.
    \end{align*}
\end{proof}
