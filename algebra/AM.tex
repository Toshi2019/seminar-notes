...
%================================================
\title{Notes on Introduction to Commutative Algebra}
\author{Toshi2019}
\date{\today 更新}
...
の2つであると言ってよい．
代数幾何で研究する環の雛形は，
体\(k\)上の多変数多項式環\(k[x_1,\dots,x_n]\)である．
代数的整数論では，有理整数環\(\zz\)が雛形となる．
これら2つのうち代数幾何の方が理論の適用範囲が広く，
近年のグロタンディークによる理論展開により，
代数的整数論の多くの部分を包含するようになっている．
可換代数はいまや，この新しい代数幾何の礎の一つになっている．
微分法が微分幾何のための道具であるのと全く同様に，
可換代数は，
代数幾何において局所的な情報を調べる完全な道具である．

この本は，オックスフォード大学での学部3年生向けの講義を発展させたものであり，
手短な入門となることがささやかな願いである．
この本は
抽象代数の初等的な講義を受けたことのある学生が読むことを念頭に置いている．
他方で，ザリスキー・サミュエルやブルバキといった，
可換代数に関するもっと大部の本の代替になることを意図するものではない．
いくつかの中心的な話題に焦点を絞り，体論などの大きな分野については触れない．
この本で扱う内容はノースコットのものよりも基礎的であり，
取り扱いもかなり異なる．この本では
最近の流行に合わせて
加群や局所化の方に重点を置いた．
...
数論における素数や
幾何学における点の共通の一般化である．
「点の近く」に注目するという幾何学的な概念には，その代数での類似として
素イデアルで環を局所化するという重要な過程がある．
したがって，局所化に関する結果が幾何学的な用語によって捉えられること
は驚くべきことではない．
...
環の定義と簡単な性質について手短に復習することから始める．

\section*{環と環準同形}
\textbf{環} (ring) は2つの演算（加法と乗法）のついた集合で
次をみたすものである．
\begin{enumerate}[1)]
    \item \(A\)は加法についてアーベル群（で\(A\)に零元\(0\)があり，任意の\(x\in A\)に対し（加法）逆元\(-x\)が存在するもの）である．
    \item 情報が結合的 (\((xy)z=x(yz)\)) であり加法について分配的 (\(x(y+z)=xy+xz\), \((y+z)x=yx+zx\)) である．
\end{enumerate}
ここでは環は\textbf{可換} (commutative) 
\begin{enumerate}[1)]\setcounter{enumi}{2}
    \item 任意の\(x,y\in A\)に対し\(xy=yx\)
\end{enumerate}
であり，\textbf{単位元} (identity element) \(1\)をもつ：
\begin{enumerate}[1)]\setcounter{enumi}{3}
    \item \(1\in A\)で任意の\(x\in A\)に対し\(x1=1x=x\)をみたすものが存在する．単位元はただひとつである．
\end{enumerate}
もののみを考える．
...
\begin{thebibliography}{20} 
    \bibitem[AM69]{AM69} Atiyah, Macdonald, \textit{Introduction to Commutative Algebra} 
