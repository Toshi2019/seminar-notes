...
\begin{prp}[{\cite[命題3.3.4]{KS90}}]\label{334}
    ...
    \begin{enumerate}[(i)]
        ...
        \begin{align*}
                ...
                \HOM(\ori_{Y/X},A_Y)&\cong \ori_{Y/X}.
        \end{align*}
...
\]
である．
\begin{RMK}[{\cite[Remark 3.3.5]{KS90}}]
    \(\vcenter{
        \xymatrix@C=8pt@R=16pt{
            Y
            \ar[rd]_{p_{Y}}
            \ar[rr]^{f}
            %\ar@{}[dr]|\square
    ...
    }\)を局所コンパクト空間の間の連続写像による可換図式とする．
    \(p_X\)と\(p_Y\)はファイバー次元がそれぞれ\(m\)と\(n\)の
    位相的しずめこみであると仮定する．
    このとき\[
        f^{!}A_{X}\cong
        \ori_{Y/S}\tens[]f^{-1}\ori_{X/S}[n-m]
    \]であり，次のようにおくのが自然である．
    \begin{equation}
        \ori_{Y/X}\coloneqq \ori_{Y/S}\tens[]f^{-1}\ori_{X/S}.
    \end{equation}
\end{RMK}
\begin{proof}
    命題\ref{334} (iii)より
    \begin{align*}
        f^{!}A_X
        &\cong f^{!}p_{X}^{-1}A_{S}\\
        &\cong f^{-1}p_{X}^{-1}A_{S}\tens[]\ori_{Y/S}\tens[]f^{-1}\ori_{Y/X}[n-m]\\
        &\cong \ori_{Y/S}\tens[]f^{-1}\ori_{Y/X}[n-m]
        %&\cong f^{!}(A_{X})\ttens[A_Y]p_{Y}^{-1}A_{S}\\
        %&\cong f^{!}(A_{X})\ttens[A_Y]p_{Y}^{-1}A_{S}
    \end{align*}
    である．
\end{proof}
...
\begin{thebibliography}{20} 
    ...
    \bibitem[Sch17]{Sch17} Pierre Schapira, 
      \textit{Microlocal Analysis and Beyond}, 
      in New Spaces in Mathematics: 
      Formal and Conceptual Reflections, 
      Cambridge University Press pp.117--152 (2021).
