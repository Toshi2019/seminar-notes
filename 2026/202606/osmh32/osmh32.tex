...
\]
がなりたつことをいう．
\(\cO(D)\coloneqq\left\{\text{\(D\)上の正則関数}\right\}\)とおく．
正則関数の実軸への制限として表される関数を実解析関数とよぶ．
\(\cA(D)\coloneqq\left\{\text{\(D\)上の実解析関数}\right\}\)とおく．
\section{フーリエ解析・境界値問題からの動機}
2次元開円盤を\(D\)で表す．すなわち
\[
...
\begin{THM}[Helgason, 1970]
  \(\cP_{s}\)は\[
    \cP\colon \cB(S^1)\to \cH_{4s(s-1)}(D)
  \]
  に拡張される．さらにこれは同型である．
\end{THM}
...
\section{超局所解析（佐藤・H\"ormanderの基本定理）}
...
\begin{DFN}
  ...
  あるとは，
  \((x;\xi)\)の錐状，すなわち\(\rrp\)不変な開近傍\(U\)で，
  \(F_{+}\)と\(F_{-}\)が\(U\)に解析接続できるものが存在する
  ことをいう．
...
\begin{thebibliography}{20} 
  ...
  \emph{複素関数入門}, 岩波講座 現代数学への入門 4, 岩波書店, 1995; 新装版, 2024. 
