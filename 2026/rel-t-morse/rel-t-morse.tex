...
% documentclass
% ------------------------
\documentclass[leqno, 11pt, dvipdfmx]{article}
...
\begin{Proposition+}[{\cite[Proposition 2.5.1]{KS90}}]\label{prop:ks2.5.1}
  Let \(X\) be a topological space, 
  \(Z\) a subspace, and \(F\) a sheaf on \(X\). 
...
\begin{Proposition+}[{\cite[Proposition 1.12.6]{KS90}}]\label{prop:ks1.12.6}
  Let \(\left(X_s,\rho_{s,t}\right)\) be 
...
and 
its functorial property under direct images of 
proper morphisms. 
First, we review the original definition 
of a microsupport. 
...
\begin{Example+}\label{eg:t-nonchar}
  ...
  \begin{enumerate}[(i)]
    ...
    function\[
      ...
      \end{cases}
    \]
    Then let us define the open subset \(U_t\)
    of \(X\) by
    \[
      U_{t}\coloneqq
      \left\{
        (x,y)\in X\smid y<\varphi_{t}(x)
      \right\}. 
    \]
    Then the family \((U_{t})_{t}\) does not satisfy 
    ...
    a locally closed subset \(S\) of \(X\) by}
    \[
      S\coloneqq 
      \left\{(x,y)\in X;\ (x+2)^{2}+(y-1)^{2}=1\right\}
      \smi 
      \left\{(-1,0)\right\}. 
    \]
    %For this \(S\) 
    We denote by \(\kk_{S}\) 
    the sheaf on \(X\) as in Section \ref{sec:review}. 
    If \(j\leqq0\), we have \(
      H^{j}_{X\smi U_{t}}(\kk_{S})_{x}=0
    \) for all \(t \geqq s\) and \(x\in Z_{s}\smi U_{t}\), 
    but the morphism \(
      H^{0}(U_{1};\kk_{S})=\kk
      \to 
      0 = H^{0}(U_{0};\kk_{S})
    ...
    Hence the condition \eqref{item:t-nonchar4} is 
    necessary. 
    \begin{figure}[htb]
      \centering
    ...
    \end{figure}
  \end{enumerate}
  Let us, in turn, give two examples 
...
\begin{proof}[Proof of Lemma \ref{clm:ind-nonchar}]
  ...
  \textcolor{red}{We show the the following statement.}
