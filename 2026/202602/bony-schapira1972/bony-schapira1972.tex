...
\usepackage{calligra}
\usepackage{mathrsfs}
%\usepackage{boondox-cal} % \mathcal と \mathscr を置き換える

% mathalpha パッケージで mathscr を置き換え
...
\DeclareMathOperator{\Map}{\mathrm{Map}} % 写像の集合
\DeclareMathOperator{\Hom}{\mathrm{Hom}} % Hom set
\DeclareMathOperator{\Mor}{\mathrm{Mor}} % Morphism set
\DeclareMathOperator{\End}{\mathrm{End}} % End 環
\DeclareMathOperator{\Aut}{\mathrm{Aut}} % Aut 群
\DeclareMathOperator{\Ker}{\mathrm{Ker}} % Kernel
\DeclareMathOperator{\Img}{\mathrm{Im}} % Image
\DeclareMathOperator{\Coker}{\mathrm{Coker}} % Cokernel
\DeclareMathOperator{\Coim}{\mathrm{Coim}} % Coimage
...
\newcommand{\Dompb}[2][]{%
  \mathop{\mathsf{D}^\mathrm{b}_{#1}}\nolimits(#2)%
}
\newcommand{\Dbconic}[1][]{\Dompb[\rrp]{#1}}
\newcommand{\Dbconstr}[1][]{\Dompb[\mathrm{c}]{#1}}





\newcommand{\CCat}{\Comp(\category)}
\newcommand{\KCat}{\Komp(\category)}
\newcommand{\DCat}{\Domp(\category)}%圏Cの複体のホモトピー圏
...
\title{偏微分方程式の正則解の存在と延長}
\author{ジャン・ミシェル・ボニー，ピエール・シャピラ}
\date{\today}
\begin{document}
\maketitle
%\tableofcontents

\(\cc^{n}\)内の正則係数微分作用素の有限族\((P_{i})\)に対し，
次の2つの問題を考える．
ひとつは，境界をまたぐ延長の問題である：\(f\)が開集合\(\varOmega\)上の
正則関数であり，また\(P_{i}f\)が境界点\(z_{0}\)の近傍に延長されるとする．
このとき境界において\(f\)は\(z_{0}\)の近傍に延長できるか．
もうひとつは，単独作用素\(P\)の場合の境界値の存在の問題である：%
\(g\)が\(\varOmega\)における\(z_{0}\)の近傍上の正則関数のとき，
\(\varOmega\)における\(z_{0}\)の近傍上の正則関数\(f\)で\(Pf=g\)をみたすものは存在するか．

第\ref{sec:4}節において，
開集合の境界が非特性的な場合に
これらの問題の肯定的解答を与える．
この結果は開集合が凸の場合，\(C^1\)級の場合，
もっと一般に\(C^{1}\)微分同相で不変な開集合のクラスで，
「錐条件」をみたすものに対し有効である．

我々の定理により，マルグランジュにより提出された
次の問題を解くことができる．
\(\varOmega\)を相対コンパクトな開集合で
境界がいたるところ非特性的なものとする．
このとき\(H^{1}(\overline{\varOmega},\sheaf{O})=0\)を示せ．
ただし作用素\(P\)を
\(\sheaf{O}(\varOmega)\)（\(\varOmega\)上の正則関数の空間）から自身への
写像としてみたときに指数が有限であるとする．

最後に，
存在・延長定理から佐藤の基本定理を自然に復元できることを述べる．
\(\rr^n\)の余接ファイバーにおいて，
\(Pu=v\)の超関数解の特異台は，
\(v\)の特異台と\(p(x,\eta)=0\)をみたす\((x,\eta)\)の集合
の合併に含まれる．

準備中の記事[1],[2]において，
以上の結果を，解析関数と超関数の枠組みで
弱 (non strictes) 双曲型方程式を解くのに応用する．


\section[記号]{記号}\label{sec:1}

\(\cc^{n}\)にエルミート内積\(
    \inner<z,\zeta>
    =\sum z_{i}\overline{\zeta}_{i}
\)を入れたものとユークリッド空間\(\rr^{2n}\)に
スカラー積\(\Re\inner<z,\zeta>\)を入れたものを同一視する．

\(S^{2n-1}\)で単位球面を表す．
\(S^{2n-1}\)の部分集合\(I\)が凸であるとは，
\(I\)から生成された錐が凸であることをいい，
\(I\)が固有であるとは，
\(I\)から生成された錐が直線をまったく含まないことをいう．
\(I\)の極とは，次で定義される凸錐である：\(I\)に属する\(\zeta\)に対し，
\(\Re\inner<z,\zeta>\leqq0\)をみたす\(z\)全体を\(I\)の極という．
\(I,I',\dots\)を\(S^{2n-1}\)の凸固有部分集合とするとき，
\(\varGamma,\varGamma',\dots\)でそれぞれの極の内部を表す．

次の形の微分作用素\(P(z,\frac{\partial}{\partial z})\)を考える：
\[
    P\left(z,\frac{\partial}{\partial z}\right)
    =
    \sum_{\left|\alpha\right|\leqq m}^{}a_{\alpha}(z)\left(\frac{\partial}{\partial z}\right)^{\alpha}
\]
ただし，\(\frac{\partial}{\partial z}=\left(\frac{\partial}{\partial z_{1}},\dots,\frac{\partial}{\partial z_{n}}\right)\)であり，
係数\(a_{\alpha}(z)\)は\(\cc^{n}\)の開集合\(U\)上の正則関数とする．
\[
    p\left(z,\zeta\right)
    =
    \sum_{\left|\alpha\right|= m}^{}a_{\alpha}(z)\zeta^{\alpha}
\]
で作用素\(P\)の主要表象を表す．

方程式\(\Re\inner<z-z_{0},\zeta>=0\)で定義された超曲面が\(z_{0}\)で特性的であるとは，
\(p(z_{0},\zeta)=0\)であることをいう．
同様に，この超平面が\(z_{0}\)において，
この型の作用素の族\((P_{i})\)に関して特性的であるとは，
任意の\(i\)に対し\(P_{i}(z_{0},\zeta)=0\)がなりたつことをいう．
\(\zeta\)が\(z_{0}\)において特性的であるともいう．


\begin{DFN}
    \(\varOmega\)を\(U\)の開集合とする．
    \(\Car(\varOmega)\)で，
    \(\varOmega\)の少なくとも1点において
    （系\((P_i)\)に関して）特性的である点方向の集合
    の閉包を表す．
\end{DFN}

\(0\)でないベクトル\(\zeta\)が\(\Car(\varOmega)\)に含まれるのは，
点列\((z_{n},\zeta_{n})\)で\(z_{n}\in\varOmega\); %
\(\zeta_{n}\to\zeta\); \(p_{i}(z_{n},\zeta_{n})=0\)を
みたすものが存在するときである．



\section[延長定理]{延長定理}\label{sec:2}

以下の結果はゼルネールの定理[13]の特別な場合である．
\ref{sec:4}で再考する．

\begin{LMM}
    \(\varOmega\)を境界が\(C^1\)級の凸開集合とする．
    \(\partial\varOmega\)の点\(z_0\)における
    法ベクトル\(\zeta\)が作用素\(P\)に関して非特性的であるとする．
    このとき，\(f\)が\(\varOmega\)で正則かつ\(Pf\)が\(z_0\)の近傍の正則関数に延長できるならば，
    \(f\)は\(z_0\)の近傍に正則に延長できる．
\end{LMM}
コーシーコワレフスカヤの定理を用いることで，\(Pf=0\)の場合に帰着できる．
以下，\(\varOmega\)の外向き単位ベクトル\(\zeta\)をとる．

このとき\(\widetilde{H}_{\varepsilon}\)で方程式\(
    \langle z-z_0,\zeta\rangle=-\varepsilon
\)の定める複素超平面を表す．
この境界は\(C^1\)級であるから，
\(z_0-\varepsilon\zeta\)を中心とする
\(\varOmega\cap \widetilde{H}_{\varepsilon}\)
に含まれる最大の球の半径は\(a\)であって，
\(\varepsilon\)が\(0\)に近づくとき\(\varepsilon\)よりも
無限に大きいものである．
\section[凸錐の場合]{凸錐の場合}\label{sec:3}

\section[存在・延長定理]{存在定理と延長定理}\label{sec:4}

\section[微分作用素の指数]{微分作用素の指数}\label{sec:5}
\section[特異台への応用]{超関数の特異台への応用}\label{sec:6}
