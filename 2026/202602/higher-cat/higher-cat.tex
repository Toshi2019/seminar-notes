...
\section{2026/02/13}
前川さんのページ\footnote{\url{https://animaegawa.github.io/blog/oo-cat}}を読んだ．
以下，そのメモ．

\paragraph{注意}
モデル圏の使用は理解の妨げになるらしい．

「アニマ=Kan複体」と安易に等式で結ばないらしい．

カノニカルな作り方はできるが，集合論的な構成は現実的でない．

\paragraph{ Disclaimer}
証明はKerodonか\cite{HTT}を見ればよいらしい．

\paragraph{ Anima}
\begin{DFN}
    小さい\(\infty\)圏\(X\)が\textbf{anima}であるとは，
    関手\(X\to\Fct(\Delta^{1},X)\)が圏同値であることをいう．

    animaからなる\(\Cat\)の充満部分圏を\(\Anima\)で表す．
\end{DFN}
...
\section{2026/02/14}
\subsection{Gaitsgory--Rozenbryum}
\cite{GR17}のメモ．

\paragraph{}
\cite[Sect. 1,2]{GR17}は\(\infty\)圏の基礎に関する簡単な復習．

ほとんどの場合，シンタックス，つまり\(\infty\)圏の使い方に焦点を当てる．
言い方を変えると，読者は\(\infty\)圏の何らかのモデル，
すなわち位相圏や単体的圏，
あるいは今日もっとも使われるモデル --- Lurie により展開された Joyal の
準圏などに習熟している必要はない．

関手のカルテシアン・余カルテシアンファイブレーション，
米田，極限・余極限，共終性，左・右Kan拡張，随伴を導入する．

\paragraph{}
\cite[Sect. 3,4]{GR17}では高次元代数の最初の一端を述べる．
モノイダル\(\infty\)圏と
モノイダル\(\infty\)圏における結合代数を導入する．
また，それに対応する可換性の概念も導入する．

さらに対応する加群の概念（すなわち，
モノイダル\(\infty\)圏に対する加群圏と結合代数に対する加群）を導入する．

\subsection{Scholze}
\cite{Sch6FF}のメモ．

\paragraph{Lecture II}

空間\(X\)に対し，(\(\infty\)) 圏\(D(X)\)を対応させるのが6FF. 
\(D(X)\)が\(\infty\)圏のとき，6FFのデータを定式化するのに少なくとも2つの流儀が知られている．
\begin{enumerate}[(1)]
  \item Liu--Zheng のやり方：Lurieの仕事に強く根ざしている．単体的集合の組合せ論に強く依存．
  \item \cite{GR17}のやり方：形式的な構造が綺麗．しかし，\((\infty,2)\)圏を用いる．とくに，色々認めてしまっている．これはLoubaton--Ruitにより証明された．
\end{enumerate}
Mannが最近，いいとこ取りをした定義を与えた．
このMannの方法を解説するのが目的．
...
外在的なものと内在的なものをそれぞれ挙げている．

\subparagraph{外在的な動機}
圏の図式の極限を考えるには，\(\infty\)圏を持ち出さないといけない．

例として，はりあわせの問題を挙げている．
例えば，スキーム\(X\)上の準連接層の\(\infty\)圏あるいは
三角圏\(\QCoh(X)\)をアフィン被覆をはりあわせて作りたいとする．
このとき，三角圏でははりあわせがうまくいかないが，
\(\infty\)圏ではうまくいく．

\subparagraph{内在的な動機}
こちらも極限に関するもの．
ただし，今度はひとつの\(\infty\)/\(\triangle\)圏の中での極限に関するものである．

極限の例としてファイバー積を考える．
これを三角圏で実現するときには，
写像錐として考えたい（ただし\([-1]\)シフトも込みの）．
ところが，写像錐は三角剣の枠組みでは，カノニカルには定まらない．

カノニカルではないが存在するという立場では，
より一般のホモトピー余極限を定義できない．
これでは例えば単体的対象の幾何的実現も扱えないし，
（オペラッドやリー代数，リー亜代数といった）代数も行えない．

\subsection{何となくの方針}
安定\(\infty\)圏を理解したい．\cite{GR17}に沿うことになるだろうが，
準圏，というか単体的集合の基本的なことは知っておきたいので，
\cite{L21}や\cite{G15}辺りをさらおうかなと．

ところで，\(\infty\)圏とかくのと無限圏とかくのではどちらがよいだろうか．
\(1\)圏・\(2\)圏のようにかくのと
同じだとすると\(\infty\)圏がよい気がするが（それと\((\infty,1)\)圏の
略記というのも踏まえるとこちらの方がよいか？），
三角圏と並列するときは無限圏のほうがよい気もする．

%===============================================
% 参考文献スペース
%===============================================
\begin{thebibliography}{20} 
    \bibitem[HTT]{HTT} Lurie, \textit{Higher Topos Theory}, 2009.
    \bibitem[G15]{G15} Groth, \textit{A Short Corse on \(\infty\)-categories}, 2015.
    \bibitem[GR17]{GR17} Gaitsgory, Rozenbryum, \textit{A Study in Derived Algebraic Geometry I}, 2017.
    \bibitem[L21]{L21} Land, \textit{Introduction to Infinity-Categories}, 2021.
    \bibitem[Sch6FF]{Sch6FF} Scholze, \textit{Six-Functor Formalisms}, preprint 2025.
\end{thebibliography}
