...
\begin{itemize}
    \item \(M\)：\(n\)次元実解析多様体
    \item \(X\)：\(M\)の複素化
    \item \(\varOmega\)：境界が解析的な\(M\)の開集合
    \item \(N\coloneqq\bd\varOmega\)
    \item \(Y\)：\(N\)の\(X\)における複素化
\end{itemize}
\(X\)の局所閉集合\(A\subset X\)に対し，（とくに\(A=\varOmega\)に対し，）
\[
...
を考える．
\[
    \mildmfun[\varOmega\mid N]\coloneqq \Rder[]{\mdiff[i]}_{!}\mpb[i]^{-1}\mfun[\varOmega\mid X]\in \Dbconic(\cotb[Y])
\]
\begin{align*}
    \mildmfun[\varOmega\mid N]&\coloneqq \Rder[]{\mdiff[i]}_{!}\mpb[i]^{-1}\mfun[\varOmega\mid X]\in \Dbconic(\cotb[Y]),\\
    \mildhfun[\varOmega\mid N]&\coloneqq \mres[{\mildmfun[\varOmega\mid N]}]{{\cotz[Y]}}\in \Dompb(Y)
\end{align*}
とおく．
...
\begin{PRP}
    \begin{enumerate}[(i)]
        \item 複体\(\mildmfun[\varOmega\mid N]\)のコホモロジーは\(0\)次に集中する．
        \item 層\((\mildmfun[\varOmega\mid N])_{\conm[N]{Y}}\)は柔軟層である．
        \item \(N\)上の層として次の自然な完全列が存在する．
        \[
            0\longrightarrow 
            \mres[\mathcal{A}_{M}]{N}\longrightarrow 
            \mildhfun[\varOmega\mid N]\longrightarrow 
            \dot{\pi}_{\ast}\mildmfun[\varOmega\mid N]
            \longrightarrow0.
        \]
    \end{enumerate}
\end{PRP}
考えたいのは，
\(\muS[k](\RHOM_{\mathcal{D}_{N}}(\mathcal{M}_{N},\mildhfun[\varOmega\mid N]))\)，
もしくは
\(\muS[k](\RHOM_{\mathcal{D}_{M}}(\mathcal{M}_{M},\Gamma_{\varOmega}\hfun[M]))\)．
%\section{a}
%===============================================
% 参考文献スペース
%===============================================
\begin{thebibliography}{20} 
  ...
  \bibitem[Kat80]{Kat80} Kiy\^omi Kataoka, 
  \textit{Micro-local theory of boundary value problems I}, 1980.
  \bibitem[Kat81]{Kat81} Kiy\^omi Kataoka, 
  \textit{Micro-local theory of boundary value problems II}, 1981.
  ...
  \bibitem[Sch88]{Sch88} Pierre Schapira, 
    ...
    in Algebraic Analysis, volume II, Academic Press (1988), 809--819. 
  \bibitem[SZ88]{SZ88} Pierre Schapira, Giuseppe Zamieri, 
    \textit{Microfunctions at the Boundary and Mild Microfunctions}, 
    Publ.~RIMS~Kyoto Univ.~24 (1988), 495--503. 
\end{thebibliography}
%===============================================
