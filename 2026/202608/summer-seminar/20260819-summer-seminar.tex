...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item \(U, V, W\)：\(M\)の開集合
  \end{itemize}
  のとき，\(U\)上の\(C^{\infty}\)函数のなす
  加群\(C^{\infty}(U)\)は自然な対象．\pause
  
  \textbf{\(C^{\infty}\)函数に対する２つの操作}：
  \begin{description}
    \item[\underbar{\textbf{制限}}]\uncover<3->{\(f\in C^{\infty}(U)\)の 
    ...
    \uncover<4->{\(U=\bigcup_{i}U_{i}\)：開被覆で，各\(U_i\)上
...
\begin{frame}
  ...
  \begin{enumerate}
      \setlength{\leftskip}{10pt}
      %\renewcommand{\labelenumi}{({\arabic{enumi}})}
      \item $\rho_{UU} = \id_{F(U)}$. \pause
      \item $W \subset V \subset U$ ならば, 
      $\rho_{WU} = \rho_{WV} \circ \rho_{VU}$，\label{enum:res}
  \end{enumerate}\pause
  さらに，\(F(\varnothing)=0\)かつ
  任意の開集合\(U\subset X\)と
  その開被覆\(\left(U_i\right)_{i\in I}\)に対して，\pause
  \begin{enumerate}[S1.]
      %\setcounter{enumi}{-1}
      \setlength{\leftskip}{10pt}
      \item \(s\in F(U)\)が各\(i\in I\)に対して\(U_i\)上\(s\rvert_{U_{i}}=0\)ならば，
      \(s=0\).\label{sheaf-cond1}\pause
      \item \(\left(s_i\right)_i\in\prod_{i}F(U_i)\)が
      各\(i,j\in I\)で\(U_i\cap U_j\ne\varnothing\)となるものに対して\(U_i\cap U_j\)上\(s_i\rvert_{U_i\cap U_j}-s_j\rvert_{U_i\cap U_j}=0\)ならば，
      \(s\in F(U)\)で各\(U_i\)上\(s\rvert_i=s_i\)となるものが存在する．\label{sheaf-cond2}
  \end{enumerate}
...
\begin{exampleblock}{例}
  ...
  \begin{enumerate}
    \item \(
        \cO_{X}\colon U\mapsto 
        \cO_{X}(U)\coloneqq\left\{\text{\(U\)上の正則函数}\right\}
    \)，\pause
    \item 
    \(
        \cD_{X}\colon U\mapsto 
        \cD_{X}(U)\coloneqq\left\{\text{\(U\)上の正則函数係数線形偏微分作用素}\right\}
    \)，
    \item \pause
    \(\cM\)：\(X\)上の正則函数係数線形偏微分方程式系（連接\(\cD_{X}\)加群）のとき，
    \[
      H^{0}\Sol[X]{\cM}\colon U\mapsto 
      H^{0}\Sol[X]{\cM}(U)\coloneqq
      \left\{u\in\cO_{X}(U)\smid \text{\(u\)は\(\cM\)の解}\right\}.
    \]
  \end{enumerate}
\end{exampleblock}
\end{frame}
...
\begin{frame}
  ...
  空間\(X\)に対し，そのコホモロジー
  \[
      H^{j}(X;G),\quad\text{\(G\)はアーベル群（\(\zz\)や\(\zz/2\zz\)など）}
  \]は重要な量．\pause
...
\begin{frame}
  ...
  \begin{NTN}[{\cite{KS90}など}]
    ...
    \begin{description}
      ...
      \item[射] コホモロジー間の同型を誘導する射（擬同型，qis）が可逆な複体の射
    \end{description}
  \end{NTN}\pause
  \(M\)：\(C^{\infty}\)多様体のとき，
...
\begin{frame}
  ...
  実解析係数線形偏微分方程式\(Pu=0\)を考える．余接束\(\pi\colon\cotb[M]\to M\)の部分集合
  \begin{itemize}
...
\end{frame}
\subsection{超局所台}
\begin{frame}\frametitle{層の超局所台 --- 方向別の特異性}
  %\(\Dompb(\kk_X)\ni F\mapsto \SS(F)\subset T^{\ast}X\)
  \(F\in\Dompb(\kk_M)\)，\(p=(x_0;\xi_0)\in\cotb[M]\)とする．

  \(F\)が\(x_0\)で\(\xi_0\)の方向に特異的なら\(p\in \muS(F)\)として
  \(F\)の超局所台\(\muS(F)\)を定義したい．

  \uncover<2->{\(x_0\)の近くの実\(C^1\)級関数\(\varphi\)で\(
      d\varphi(x_0)=\xi_0
  \)となるものを考える．}

  \uncover<3->{\(x_0\)の近くの\(\xi_0\)の定める半空間のコホモロジーが延長できる，
  つまり，}
 %各次数のコホモロジーにおける``方向\(p\)の定める半空間への制限''
  \uncover<4->{\[
    H^{j}F_{x_0}\cong \indlim[B\ni x_{0}]H^{j}(B;F)
    \to
    \indlim[B\ni x_{0}]H^{j}(B\cap\{x\smid\varphi(x)<\varphi(x_0)\};F).
  \]
  ...
  \begin{figure}[htb]
    ...
    \begin{tikzpicture}[scale=0.5]
      ...
      % 半近傍
      \fill[blue!25, domain=(-\t-3):(\t-3), samples=200, visible on=<3->]
        (-\t-3, 0)
        -- plot (\x, {-sqrt(\t*\t - (\x+3)*(\x+3))})
        -- (\t-3, 0)
        -- cycle;
      \draw[dashed, very thick, visible on=<2->] (-3,0) circle (1.5);

      % 近傍
      \fill[blue!25, visible on=<4->] (3,0) circle (\t);
      \draw[dashed, very thick, visible on=<4->] (3,0) circle (1.5);

      %------------------
      % 軸・ラベル
      %------------------

      \node[below, visible on=<1->] at (-3,0) {\(x_0\)};
      \draw[->, >=stealth, very thick, visible on=<1->] (-3,0)--(-3,\t+0.5) node[above]{\(\xi_0\)};
      \node[below, visible on=<3->] at (-7.5,-0.5) {\(B\cap\{\varphi(x)<\varphi(x_0)\}\)};

      \node[below, visible on=<4->] at (3,0) {\(x_0\)};
      \draw[->, >=stealth, very thick, visible on=<4->] (3,0)--(3,\t+0.5) node[above]{\(\xi_0\)};
      \node[below, visible on=<4->] at (5.2,-0.5) {\(B\)};
      
      \draw[->, >=stealth, semithick, visible on=<4->] (-1,0)--(1,0);
      \node[below, visible on=<4->] at (0,0) {延長？};
    \end{tikzpicture}
  \end{figure}
\end{frame}

\begin{frame}\frametitle{層の超局所台の定義と微分方程式論との関係}
  \begin{DFN}[層の超局所台]
    \(p\notin \muS(F)\)であるとは，
    \(p\)の\(\cotb[M]\)における開近傍\(U\)で次をみたすものが存在することをいう．
    任意の\(x_1\in X\)と
...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.45\hsize}
      ...
      % 近傍
      \fill[purple!25, domain=(-\t):(\t), samples=200, visible on=<3->]
        (-\t, 0)
        -- plot (\x, {-sqrt(\t*\t - (\x)*(\x))})
        -- (\t, 0)
        -- cycle;
      \fill[purple!25, visible on=<2>] (0,0) circle (\t);
      \draw[dashed, very thick,visible on=<2->] (0,0) circle (\t);
      \node[visible on=<2>] at (\t, 1) {\(U\)};
      \node[visible on=<3>] at (0.8\t, -1) [right]{\(U\cap \{y<0\}\)};
    ...
    \begin{column}{0.45\hsize}
      ...
      \uncover<3->{一方，
      \[
        H^{0}(U\cap \left\{y <0\right\};\kk_{Z})=\kk.
      \]}
      \uncover<4->{したがって，これらは同型．他の次数も同型なので
      \[
        (0;(0,1))\notin \muS[](\kk_{Z}).
      \]}
    \end{column}
  ...
  \begin{columns}
    ...
    \begin{column}{0.45\hsize}
      ...
      \[
        H^{j}(U;\kk_{Z})=\kk.
      \]}
      \uncover<3->{一方，開上半空間は\(Z\)と交わらないので
      \[
...
\begin{frame}
  ...
  \begin{figure}[htb]
    ...
    \begin{tikzpicture}[scale=0.75]
      ...
      \node at (-1,0) [below]{\(-1\)};
    \end{tikzpicture}
    \begin{tikzpicture}[scale=0.75, visible on =<2->]
      \def\t{0.7}
      \def\xmax{3.5}
      \def\ymin{-3}
      \def\ymax{3}
...
\begin{frame}\frametitle{打切り超局所台 --- 次数依存の方向別特異性}
    \begin{alertblock}{注意}
    ...
    そこで，超局所台を精密化することで次の定義を得る．\pause
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item<2-> \(k\geqq0\)のとき，\(\muS[k](\kk_{[-1,\infty)})=\muS[](\kk_{[-1,\infty)})\).
  \end{itemize}
  \begin{figure}[htb]
    \centering
...
\begin{frame}
  ...
  \begin{itemize}
    \item \(k\leqq-1\)のとき，\(\muS[k](\kk_{(-1,\infty)})=\varnothing\),
    \item<2-> \(k=0\)のとき，\(\muS[k](\kk_{(-1,\infty)})=\supp(\kk_{(-1,\infty)})=[-1,\infty)\).
    \item<3-> \(k\geqq1\)のとき，\(\muS[k](\kk_{(-1,\infty)})=\muS[](\kk_{(-1,\infty)})\).
  \end{itemize}
...
\begin{frame}
  ...
  \end{THM}
  すなわち，初期面\(\{\varphi=0\}\)が\(P\)に関して非特性的なら，
  \(Pu=0\)の局所解は，あったとすれば，ただ一つ！\pause
  ...
  \(x\in X\)の開近傍\(B\)で次が単射ということ．
  \[
    H^{0}(B;F)\to H^{0}(B\cap \left\{\varphi<0\right\};F).
  \]\pause

  我々の主結果１は，
  \textcolor{red}{Holmgrenの一意性定理の大域化}になっている！
\end{frame}
\subsection{主結果２}
...
\begin{frame}
  ...
  \begin{THM}[非特性変形補題 {\cite{KS90}}]
    ...
    \begin{enumerate}[(i)]
      ...
      \uncover<2->{任意の\(t\in\rr\)に対し\(U_{t}=\bigcup_{s<t}U_{s}\)である．}
      \item \label{item:nonchar2}
      \uncover<3->{任意の実数\(s\leqq t\)に対し\(
        \overline{U_{t}\setminus U_{s}}\cap \supp{F}
      \)はコンパクトである．}
      \item \label{item:nonchar3}
      \uncover<4->{任意の実数\(s\leqq t\)と任意の点\(x\in Z_{s}\setminus U_{t}\)に対し，
      \(
...
\begin{frame}
  ...
  \end{alertblock}\pause
    ...
    \(
      ...
      \right\}
    \)とする．%次ページのように層\(\kk_{S}\)を定めると，
    \((U_{t})_{t}\)%と\(\kk_{S}\)
    は\eqref{item:t-nonchar1}--\eqref{item:t-nonchar2}をみたす．
      \begin{figure}[htb]
      \centering
      \begin{tikzpicture}[scale=0.50]
        ...
        % y<0 の部分
        \fill[green!30,visible on=<3->] (-\xmax,0) rectangle (\xmax,\ymin);
        \fill[blue!25,visible on=<2->] (-\xmax,0) rectangle (\xmax,\ymin);

        % 三角形部分 y < t(2-|x|)
        \fill[green!30,visible on=<3->]
          (-2,0) -- (0,{2*\u}) -- (2,0) -- cycle;

        \fill[blue!25,visible on=<2->]
          (-2,0) -- (0,{2*\t}) -- (2,0) -- cycle;
        ...
        % 折れ線
        \draw[very thick,dashed,visible on=<3->] (-2,0)--(0,{2*\u})--(2,0);
        % 折れ線
        \draw[very thick,dashed,visible on=<2->] (-2,0)--(0,{2*\t})--(2,0);
        ...
        \node at (2.4,0.8) {\(U_t\)};
      \end{tikzpicture}
      \begin{tikzpicture}[scale=0.50,visible on=<3->]
        \def\t{0.7}
        \def\xmax{3.5}
        \def\ymin{-1}
        \def\ymax{3}
...
\begin{frame}
  \frametitle{うまくいかない例（続）}
    \(X\)の局所閉集合\(S\)を次で定める．
    \[
      S\coloneqq \left\{(x,y)\in X;\ (x+2)^{2}+(y-1)^{2}=1\right\}\setminus \left\{(-2,0)\right\}. 
    \]
    \(S\)上の定数層の\(X\)への零による延長\(\kk_{S}\)を考える．

    \uncover<2->{\(j\leqq0\)のとき，
    任意の実数\(s\leqq t\)と任意の点\(x\in Z_{s}\setminus U_{t}\)に対し\(
      H^{j}_{X\setminus U_{t}}(\kk_{S})_{x}=0
    \)で\eqref{item:t-nonchar3}成立．}
...
\begin{frame}
  ...
  \[\vcenter{\xymatrix@C=36pt@R=10pt{
    ...
    &
    \txt{打切り超局所Morseの補題\\
    主結果１[\textcolor{darkblue}{O.}]\\
    \(H^{j}(M_b;F)\to H^{j}(M_a;F)\)\\
    \(j<k\)で同型，\(j=k\)で単射}
    &\txt{打切り非特性変形補題\\主結果２[\textcolor{darkblue}{O.}]\\\(
    H^{j}\left(\bigcup\limits_{s\in\rr}U_{s};F\right)
    \to
    H^{j}\left(U_{t};F\right)\)\\\(
    j<k\)で同型，\(j=k\)で単射}
...
\begin{frame}[fragile]
  \frametitle{相対版の準備：接束の基本図式}
    次の図式が基本的．
    \[
    \begin{tikzcd}[column sep=22pt, row sep=26pt, ampersand replacement=\&]
      |[visible on=<2->]| TM
        \arrow[rr, "df", bend left=30, visible on=<3->]
        \arrow[r, "f'", visible on=<5->]
        \arrow[rd, "\tau"', visible on=<2->]
      \& |[visible on=<4->]| {M\pb[N]TM} % ← ここを中括弧 {} で囲むのが非常に重要です
        \arrow[r, "f_{\tau}", visible on=<4->]
        \arrow[d, "\tau", visible on=<4->]
        \arrow[dr, phantom, "\square"{description,pos=0.4}, visible on=<4->]
        %\arrow[dr, phantom, "\square", visible on=<3->]
      \& |[visible on=<2->]| TN
        \arrow[d, "\tau", visible on=<2->]
      \\
      \& M \arrow[r, "f"'] \& N.
    \end{tikzcd}
    \]\pause
    ここで，
    \begin{itemize}
      \item<2-> \(\tau(x;v)=x\)：底空間への射影，
      \item<3-> \(df(x;v)=(f(x);df_{x}(v))\)：底空間の射\(f\)と，\(f\)の\(x\)における微分の対，
      \item<4-> \(f_{\tau}(x,(f(x);w))=(f(x);w)\)：第\(2\)射影（\(M\tpb[N]TN\)は\(f^{\ast}TN\)ともかく），
      \item<5-> \(f'(x;v)=(x,f(x);df_{x}(v))\)：\(M\)上のベクトル束の射．
    \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{相対版の準備：余接束の基本図式}
    余接束では\(M\)上のベクトル束の射を転置する．
    \[
    \begin{tikzcd}[column sep=22pt, row sep=26pt, ampersand replacement=\&]
      |[visible on=<2->]| {\cotb[M]}
        %\arrow[rr, "df", bend left=30, visible on=<3->]
        \arrow[rd, "\pi"', visible on=<2->]
      \& |[visible on=<3->]| {M\pb[N]\cotb[N]} % ← ここを中括弧 {} で囲むのが非常に重要です
        \arrow[l, "{\mdiff[f]}"', visible on=<4->]
        \arrow[r, "{\mpb[f]}", visible on=<3->]
        \arrow[d, "\pi", visible on=<3->]
        \arrow[dr, phantom, "\square"{description,pos=0.4}, visible on=<3->]
        %\arrow[dr, phantom, "\square", visible on=<3->]
      \& |[visible on=<2->]| {\cotb[N]}
        \arrow[d, "\pi", visible on=<2->]
      \\
      \& M \arrow[r, "f"'] \& N.
    \end{tikzcd}
    \]\pause
    ここで，
    \begin{itemize}
      \item<2-> \(\pi(x;\xi)=x\)：底空間への射影，
      %\item<3-> \(df(x;v)=(f(x);df_{x}(v))\)：底空間の射\(f\)と，\(f\)の\(x\)における微分の対，
      \item<3-> \(\mpb[f](x,(f(x);\eta))=(f(x);\eta)\)：第\(2\)射影（\(M\tpb[N]\cotb[N]\)は\(f^{\ast}\cotb[N]\)ともかく），
      \item<4-> \(\mdiff[f](x,f(x);\eta)=(x;f^{\ast}\eta)\)：各点\(x\)での微分の転置．
    \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{相対版の準備：順像}
  多様体の射に沿って，もとの層をおしだすことができる．
  \begin{exampleblock}{順像}
    \(F\in\Dompb(\kk_M)\)と多様体の射\(f\colon M\to N\)に対して，
    \[
      \Rder[j]{f_{\ast}}F\in \Mod(\kk_{N})
    \]
    は，\(
      V\mapsto H^{j}(f^{-1}(V);F),\ (V\subset N)
    \)
    から定まる層．
  \end{exampleblock}\pause
  逆像は開うめこみの場合，次のようになる．
  \begin{exampleblock}{開うめこみの逆像}
    開うめこみ\(
    \iota\colon U\hookrightarrow M
    \)に対し，逆像\(\iota^{-1}F\)を次で定義．
    \[
      V\mapsto \iota^{-1}F(V)\coloneqq F(V),\ (V\subset U).
    \]
  \end{exampleblock}
\end{frame}
...
\begin{frame}
  ...
  \(N=\pt\)（1点）のとき，
  定値写像\(f\colon M\to \pt\)に関する順像は次のようになる．\[
    \Rder[j]{f_{\ast}}F=H^{j}(M;F),
    \quad
    \Rder[j]{{f_{t}}_{\ast}}\iota^{-1}_{t}G
    =H^{j}(M_{t};F)
  \]
  \(\mres[f]{M_{<b}}\colon M_{<b}\to \pt\)
  を考えれば，主結果１の絶対版が得られる．
  \begin{THM}[O., 打切り版超局所Morseの補題]
    \textcolor{brown}{\(k\in\zz\)とする．}
    \(a\leqq\psi(x)< b\)ならば
...
\begin{frame}
  ...
  \]
  の同型と単射を示す．
  次の変形族\((U_{s})_{s\in\rr}\)を用いればよい．\pause
  \begin{itemize}
    \item 関数\(\chi_{s}\colon U\to \rr\)を
    次で定め\(\gamma_{s}\coloneqq\left\{
      (x,x')\in U\times \rr \smid \chi_{s}(x)\leqq x'
    \right\}\)とおく．\[
      \chi_{s}(x)\coloneqq
      \begin{cases}
        t & (s\leqq t),\\
      ...
      \end{cases}
    \]\pause
    \item 開集合\(U_{s}\)を次で定める．（\(W_{1/8s}\)の定義は次のスライド）
    \[
      U_{s}\coloneqq \begin{cases}
        \left\{(x,x')\in U\times \rr \smid 
