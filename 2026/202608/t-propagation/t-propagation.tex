...
\maketitle
\section*{はじめに}
\section{{\cite[Theorem~5.2.1]{KS90}}について}
\(E\)を有限次元実ベクトル空間とし，\(X\)を\(E\)の開集合とする．
\(F\in\Dompb(X)\)とする．

\begin{PRP}
  \(U\)を\(X\)の開集合とする．\(\gamma\)を\(E\)の閉固有凸錐で\(0\)を含むものとする．
  \(\varOmega_{0}\)，\(\varOmega_{1}\)を\(E\)の\(\gamma\)~開集合で次をみたすものとする．
  \begin{align}
      \muS[k](F)\cap (U\times\Int[]\gamma^{\circ a})=\varnothing,\\
      \varOmega_{0}\subset \varOmega_{1}\quad\text{かつ}\quad \varOmega_{1}\smi \varOmega_{0}\subset U,\\
      \text{任意の\(x\in\varOmega_{1}\)に対し\((x+\gamma)\smi \varOmega_{0}\)がコンパクトである．}
  \end{align}
\end{PRP}
\begin{LMM}
  \(\varOmega_{0}\)が次の形の半空間とする．
  \[
    \varOmega_{0}\coloneqq\left\{x\in E\smid \inner<x,\xi_{0}><c\right\}.
  \]
  このとき，上の命題の結論は正しい．
\end{LMM}
