...
% documentclass
% ------------------------
\documentclass[leqno, 12pt, a4paper, dvipdfmx]{jsarticle}
...
\begin{proof}
  ...
  \(f=q\circ \tilde{f}\). 
  \begin{proof}
    Let \(K\) be a compact subset of \(X\times \rr\). 
    If one sets \(K_{1}\coloneqq q(K)\) and \(K_{2}\coloneqq \tilde{\varphi}(K)\), 
    then \(K\subset K_{1}\times K_{2}\) and 
    we have \[
        \tilde{f}^{-1}(K)
        \subset 
        \tilde{f}^{-1}(K_{1}\times K_{2}).
    \]
    Thus it suffices to show that \(
        \tilde{f}^{-1}(K_{1}\times K_{2})\cap \supp{G}
    \) is compact. 
    Now we have \(
        \tilde{f}^{-1}(K_{1}\times K_{2})
        =
        f^{-1}(K_{1})\cap \varphi^{-1}(K_{2})
    \). Since \(K_{2}\) is a compact subset of \(\rr\), 
    we may assume \(K_{2}=\closedintvl{a,b}\) 
    for some \(a\leqq b\). 
    In this case, it follows that
    \[
        f^{-1}(K_1)\cap \varphi^{-1}(\closedintvl{a,b})\cap \supp{G}
        \subset
        f^{-1}(K_1)\cap \varphi^{-1}(\lopenrclosedintvl{-\infty,b})\cap \supp{G}
    \]
    and by the hypothesis that \(\overline{Y_{b}}\cap \supp{G}\) is proper over \(X\), 
    \(f^{-1}(K_{1})\cap \overline{Y_{b}}\cap \supp{G}\) is compact. 
    Then the result follows. 
  \end{proof}
  Let us denote by \(i_t\) the open embedding \(
    X\times\lopen -\infty, t\ropen\hookrightarrow X\times \rr
  \) and by \(\tilde{i}_t\) the closed embedding \(
  ...
  % これらの図式は tikz-cd を使うと自動で美しい余白が取れます
  \[
  \begin{tikzcd}[row sep=30pt, column sep=30pt]
    Y
      \arrow[r, "\tilde{f}"]
      \arrow[rd, phantom, "\square"]
    &
    X\times\rr
      \arrow[r, "q"]
    &
  ...
  \begin{tikzcd}[row sep=30pt, column sep=30pt]
    ...
    X\\
    \widetilde{Y}_{t}
      \arrow[u, hook, "\tilde{j}_t"]
      \arrow[r, "\check{f}_{t}"]
    &
    X\times \lopenrclosedintvl{-\infty,t}
      \arrow[u, hook, "\tilde{i}_{t}"']
      \arrow[ur, "\tilde{q}_{t}"']
    &
  \end{tikzcd}
  \]
  ...
  このとき，
  \(U_t\colon U\times \openintvl{-\infty,t}\)として\((U_t)\)を
  考えても打切り版非特性変形補題は使えない．
\end{proof}
