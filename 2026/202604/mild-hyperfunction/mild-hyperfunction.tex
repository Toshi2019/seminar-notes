...
\date{\today\footnote{First draft: 2026/04/20}}
\begin{document}
\maketitle
\section{はじめに}
mild 超関数のモチベーションとかのサーベイ．

\section{流体のやつ}
\(x\in\rr^n\)，\(y\in\rr\)とする．
超関数\(u(x,y)\)が\(y>0\)で定義されているときに，
\(y=0\)での値，つまり境界値\(u(x,+0)\)は一般には定まらない．
\(u(x,+0)\)が\(x_0\in\rr^n\)のある近傍\(N\subset\rr^n\)で
定義されるとき，
\(u(x,y)\)は\(\left\{ y=0 \right\}\)の正の側から
\((x_0,0)\)でmildであるという．
このとき，\(u(x,y)\)を\(y\geqq0\)で定義された関数\(\mathrm{ext}(u)(x,y)\)に拡張できる．

\section{Yamazaki}
We remark that in the case of a single differential equation, 
these results are given by 
Komatsu--Kawai [25] and Schapira [31]. 
Kataoka [23] extended to the case of microfunction solutions 
to a microdifferential equation by using the theory of 
mild microfunctions. 
Further, these facts can be extended to higher-codimensional 
boundary value problem for systems of 
differential equations (see for example Oaku--Yamazaki [28]). 
Moreover, elliptic and hyperbolic boundary value problems 
for systems are studied by Kashiwara--Kawai [12] and 
Kashiwara--Schapira [17] respectively from 
the microlocal point of view.
