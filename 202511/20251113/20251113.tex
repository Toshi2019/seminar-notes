...
\section{完備Riemann多様体 ({\cite[sec.4.3]{今野13}})}

\((M,g)\)をRiemann多様体とする．
以下，接続はすべてLevi-Civita接続とする．

\begin{DFN}
    \((M,g)\)が完備であるとは，
    \((M,d_{g})\)が距離空間として完備であることをいう．
\end{DFN}

\begin{leftbar}
\begin{THM}
    \((M,g)\)を連結な完備Riemann多様体とする．
    \begin{enumerate}[(1)]
        \item 次の条件は同値である．\label{cond:HR}
        \begin{enumerate}[(a)]
            \item \(p\in M\)で\(\Exp_{p}\)が\(T_{p}M\)全体で定義されるものが存在する．\label{item:HRa}
            \item \(K\)が\((M,d_{g})\)の有界閉集合ならば，\(K\)はコンパクトである．\label{item:HRb}
            \item \((M,g)\)は完備である．\label{item:HRc}
            \item 任意の\(p\in M\)に対し，\(\Exp_{p}\)が\(T_{p}M\)全体で定義される\label{item:HRd}
        \end{enumerate}
        \item \eqref{cond:HR}の同値な条件が成り立つとき，
        任意の\(p,q\in M\)に対し，
        \(p\)と\(q\)を結ぶ最短測地線が存在する．
    \end{enumerate}
\end{THM}
...
証明は次の順におこなう．
\begin{itemize}
    \item \eqref{item:HRa}\(\Rightarrow\)\eqref{item:HRb}（補題\ref{lmm4.3.2}を用いる．）
    \item \eqref{item:HRb}\(\Rightarrow\)\eqref{item:HRc}（先週（11/6）やった．）
    \item \eqref{item:HRc}\(\Rightarrow\)\eqref{item:HRd}
    \item \eqref{item:HRd}\(\Rightarrow\)\eqref{item:HRa}（これは当たり前なので省略）
\end{itemize}

\begin{LMM}\label{lmm4.3.2}
    \((M,g)\)を連結な完備Riemann多様体とする．
    \(p\in M\)に対し，\(\Exp_{p}\)が\(T_{p}M\)全体で定義されるとする．
    このとき，任意の\(q\in M\)に対し\(v\in T_{p}M\)で\[
        c_{(p,v)}(t)=\Exp_{p}(t,v)\quad (t\in[0,1])
    \]が\(p\)と\(q\)を結ぶ最短線となるものが存在する．
\end{LMM}
\appendix
\section{本編で使う主張}
\begin{THM}[{\cite[定理4.2.1]{今野13}}]
    \(p\in M\)と実数\(r>0\)に対し\[
        V_{r}(p)\coloneqq 
        \left\{v\in T_{p}M;\left|v\right|<r\right\}
    \]とする．

    (2) 
    任意の\(p\in M\)に対し，\(\delta>0\)で\begin{align*}
        \Exp_{p}(V_{\delta}(p))\subset M: \text{open}\\
        \mres[\Exp_{p}]{V_{\delta}(p)}\colon V_{\delta}(p)\to \Exp_{p}(V_{\delta}(p)): \text{diffeo.}
    \end{align*}
    となるものが存在する．

    (3) 
    \(K\)を\(M\)のコンパクト集合とする．(2)の\(\delta>0\)は一様にとれる．

    (5) 
    \(B_{r}(p)\coloneqq \Exp_{p}(V_{r}(p))\)とする．
    \(\delta\)を(2)の条件をみたすものとする．
    このとき，実数\(\delta>r>0\)で\[
        B_{r}(p)= \Exp_{p}(V_{r}(p))
    \]をみたすものが存在する．
\end{THM}
%\section{aa}
%===============================================
