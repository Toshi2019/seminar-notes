...
% documentclass
% ------------------------
\documentclass[11pt,leqno, a4paper, dvipdfmx]{jsreport}
...
\title{シンプレクティック幾何まとめノート}
\author{}
\date{\today 更新\footnote{2024/07/29 書きはじめ}}
\begin{document}
\maketitle
...
\begin{DFN}
    シンプレクティックベクトル空間\(\left(V,\omega\right)\)とは，
    有限次元ベクトル空間\(V\)に特別な双線形形式\(\omega\)で
    反対称かつ非退化なもの，すなわち
    \begin{equation}
        \omega(u,v)=-\omega(v,u),\quad u,v\in V
    \end{equation}
    かつ
    任意の\(u\ne 0\in V\)に対し，
    \(v\in V\)で\(\omega(u,v)\ne0\)となるものが存在する
...
この非退化性は，写像
\begin{equation}
    V\to V^{\ast},\quad v\mapsto \omega(v,\cdot)
\end{equation}
が\(V\)から\(V^{\ast}\)の上への線形同型であるという条件と同値である．
\begin{CMT}
    任意の\(u\ne 0\)に対し\(v\in V\)で\(\omega(u,v)\ne0\)となるもの
    が存在するとする．\(v\)に対し
    \(\omega(v,\cdot)=0\)とする．
\end{CMT}

\section{\((\rr^2,\omega_{0})\)におけるシンプレクティック微分同相写像とハミルトンベクトル場}
...
\(f\colon(M,\omega)\hookrightarrow (N,\tau)\)が
